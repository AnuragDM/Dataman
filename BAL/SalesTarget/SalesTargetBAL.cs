   ...
   public class SalesTargetBAL
    {
       public int Insert(string TSalesTargetFromHODocId, int PartyTypeId, decimal JanValue, decimal FebValue, decimal MarValue, decimal AprValue, decimal MayValue, decimal JunValue, decimal JulValue, decimal AugValue, decimal SepValue, decimal OctValue, decimal NovValue, decimal DecValue, int SMID, decimal Sno, int AssignedByID, string SalesYear, string Itemtype, string TargetFor, string Unit)
       {
           ...
           dbParam[20] = new DbParameter("@Itemtype", DbParameter.DbType.VarChar, 50, Itemtype);
           dbParam[21] = new DbParameter("@TargetFor", DbParameter.DbType.VarChar, 50, TargetFor);
           dbParam[22] = new DbParameter("@Unit", DbParameter.DbType.VarChar, 50, Unit);
       ...
       public int Inserttargetfordist(string TSalesTargetFromHODocId, int PartyTypeId, decimal JanValue, decimal FebValue, decimal MarValue, decimal AprValue, decimal MayValue, decimal JunValue, decimal JulValue, decimal AugValue, decimal SepValue, decimal OctValue, decimal NovValue, decimal DecValue, int DistID, decimal Sno, int AssignedByID, string SalesYear, string Itemtype, string TargetFor,string Unit)
       {
           DbParameter[] dbParam = new DbParameter[23];
           dbParam[0] = new DbParameter("@TSalesTargetFromDistDocId", DbParameter.DbType.VarChar, 30, TSalesTargetFromHODocId);
           dbParam[1] = new DbParameter("@PartyTypeId", DbParameter.DbType.Int, 4, PartyTypeId);
           dbParam[2] = new DbParameter("@JanValue", DbParameter.DbType.Decimal, 4, JanValue);
           dbParam[3] = new DbParameter("@FebValue", DbParameter.DbType.Decimal, 4, FebValue);
           dbParam[4] = new DbParameter("@MarValue", DbParameter.DbType.Decimal, 4, MarValue);
           ...
           dbParam[21] = new DbParameter("@TargetFor", DbParameter.DbType.VarChar, 50, TargetFor);
           dbParam[22] = new DbParameter("@Unit", DbParameter.DbType.VarChar, 50, Unit);
           DbConnectionDAL.ExecuteNonQuery(CommandType.StoredProcedure, "sp_SalesTragetFromDist_ups", dbParam);
           return Convert.ToInt32(dbParam[19].Value);
       }
