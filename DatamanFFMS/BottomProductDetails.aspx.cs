...
{
    public partial class BottomProductDetails : System.Web.UI.Page
    {
        ...
        private void fillRepeter()
        {
            ...
            sql = "SELECT TOP 99 convert(int,sum(os.Qty)) AS qty, convert(int,sum(os.rate)) AS rate,convert(decimal,sum(os.Qty*os.Rate)) AS amount,mp.PartyName FROM TransOrder1 os LEFT JOIN mastparty mp ON os.PartyId=mp.PartyId WHERE ItemId='" + item + "' and os.VDate>='" + From + "' and os.VDate<='" + To + "' AND mp.PartyDist=0 and os.smid in (SELECT smid FROM MastSalesRep WHERE SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + outputStr + ")) and Active=1)  GROUP BY mp.PartyName,rate Union ALl SELECT TOP 99 convert(int,sum(os.Qty)) AS qty, convert(int,sum(os.rate)) AS rate,convert(decimal,sum(os.Qty*os.Rate)) AS amount,mp.PartyName FROM Temp_TransOrder1 os LEFT JOIN mastparty mp ON os.PartyId=mp.PartyId WHERE ItemId='" + item + "' and os.VDate>='" + From + "' and os.VDate<='" + To + "' AND mp.PartyDist=0 and os.smid in (SELECT smid FROM MastSalesRep WHERE SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + outputStr + ")) and Active=1)  GROUP BY mp.PartyName,rate";
        ...
        protected void btnExport(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=BottomProductDetail.csv");
