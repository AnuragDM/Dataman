...
{
    public partial class ExpenseAdd_Old : System.Web.UI.Page
    {
        ...
        private void BindItemGrp()
        {
            string str = "SELECT ItemId,ItemName FROM MastItem where Itemtype='MATERIALGROUP' and Active=1 order by ItemName";
            chkbxprodgrp.DataSource = DbConnectionDAL.getFromDataTable(str);

            chkbxprodgrp.DataTextField = "ItemName";
            chkbxprodgrp.DataValueField = "ItemId";
            chkbxprodgrp.DataBind();
        }
        private void BindClient()
        {
            if (ddlCity.SelectedIndex > 0)
            {
                string str = "select PartyId,PartyName from MastParty where Active=1 and CityId=" + ddlCity.SelectedValue + " order by PartyName";
                fillDDLDirect(ddlparty, str, "PartyId", "PartyName", 1);
            }
        }
        private void BindConvClient()
        {
            string city1 = ddlConvcity.SelectedValue;
            if (city1 == "0") city1 = Request.Form[ddlConvcity.UniqueID];

            if (!string.IsNullOrEmpty(city1))
            {
                string str = "select PartyId,PartyName from MastParty where Active=1 and cityid=" + city1 + " order by PartyName";
                fillDDLDirect(ddlparty, str, "PartyId", "PartyName", 1);
                // ddlConvcity.SelectedValue = city1;
            }
        }
...
from MastLocalConveyanceLimt where DesId=" + Settings.Instance.DesigID + " and CityTypeId=(select CityType from MastArea where AreaId=" + ddlConvcity.SelectedValue + " and AreaType='city')),0) AS ConveyanceAmt  ";
        ...
        private void ClearControls()
        {
            ...
            ConvBillNum.Text = "";
            ConvClaimAmt.Text = "";
            ConvPartyremarks.InnerText = "";
            ConvRemarks.Text = "";
            txtDist.Text = "";
            gdvprodgrp.DataSource = null;
            gdvprodgrp.DataBind();
        ...
        protected void btnConvSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Convert.ToBoolean(Session["statusExp2"]))
                {
                    ...
                    if (ViewState["ClientProduct"] != null)
                    {
                        dtpg = (DataTable)ViewState["ClientProduct"];
                        if (dtpg.Rows.Count <= 0)
                        {
                            ...
                            lblConverror.Attributes.Remove("class");
                            this.ModalPopupExtender2.Show();
                            lblConverror.InnerHtml = "* Please enter atleast one Client and Item group";
                            return;
                        }
                    }
                    else
        ...
        private void GetValOnEdits(Int32 ExpenseDetailId, int flag)
        {
            try
            {
                ...
                string str = "select * from ExpenseDetails where ExpenseDetailId=" + ExpenseDetailId + "";
                DataTable dt = new DataTable();
                dt = DbConnectionDAL.GetDataTable(CommandType.Text, str);
                if (dt.Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        else { chkConvAllow0.Checked = false; }
                        ...
                        //}
                        gdvprodgrp.DataSource = (DataTable)ViewState["ClientProduct"];
                        gdvprodgrp.DataBind();
        ...
        {
            DataTable dt = new DataTable();
            string strQqy = "select Ed.ExpenseDetailID,ED.BillAmount,ED.BillDate,ED.BillNumber,ED.CityID,ED.ClaimAmount,ED.FromCity,ED.FromDate,ED.IsSupportingAttached,ED.Remarks,Ed.ToCity,ED.ToDate,ED.TravelModeID,MET.Name,MAC.AreaName,MET.ExpenseTypeCode,Case ED.IsSupportingAttached when 1 then 'Yes' else 'No' end as IsSupportingAttached1 from ExpenseDetails  ED inner join MastExpenseType MET on ED.ExpenseTypeID=MET.Id Inner Join MastArea MAC on Ed.CityID=MAC.AreaId where ED.ExpenseGroupID=" + ViewState["ExpGrpId"] + " group by met.Name,Ed.ExpenseDetailID,ED.BillAmount,ED.BillDate,ED.BillNumber,ED.CityID,ED.ClaimAmount,ED.FromCity,ED.FromDate,ED.IsSupportingAttached,ED.Remarks,Ed.ToCity,ED.ToDate,ED.TravelModeID,MAC.AreaName,MET.ExpenseTypeCode,ED.IsSupportingAttached" +
                " Union " + " Select Ed.ExpenseDetailID,ED.BillAmount,convert(varchar(12),ED.BillDate,106) as BillDate,ED.BillNumber,ED.CityID,ED.ClaimAmount,ED.FromCity,ED.FromDate,ED.IsSupportingAttached,ED.Remarks,Ed.ToCity,ED.ToDate,ED.TravelModeID,MET.Name,MAC.AreaName,MET.ExpenseTypeCode,Case ED.IsSupportingAttached when 1 then 'Yes' else 'No' end as IsSupportingAttached1 from ExpenseDetails  ED inner join MastExpenseType MET on ED.ExpenseTypeID=MET.Id Inner join MastArea MAC on Ed.FromCity=MAC.AreaId where ED.ExpenseGroupID=" + ViewState["ExpGrpId"] + " group by met.Name,Ed.ExpenseDetailID,ED.BillAmount,ED.BillDate,ED.BillNumber,ED.CityID,ED.ClaimAmount,ED.FromCity,ED.FromDate,ED.IsSupportingAttached,ED.Remarks,Ed.ToCity,ED.ToDate,ED.TravelModeID,MAC.AreaName,MET.ExpenseTypeCode,ED.IsSupportingAttached";
        ...
        protected void Btnadd_Click(object sender, EventArgs e)
        {
            ...
            lblvalidate.InnerText = string.Empty;
            bool chkdval = false;
            if (ViewState["ExpCode"].ToString().ToUpper() != "CONVEYANCE")
            {
                trconv.Attributes.Remove("style");
            }
            if (ViewState["ClientProduct"] != null)
            {
                dtbl = (DataTable)ViewState["ClientProduct"];
            }
            else
            {
                dtbl.Columns.Add("Client");
                dtbl.Columns.Add("ProdGrp");
                dtbl.Columns.Add("ClientID");
                dtbl.Columns.Add("ProdGrpID");
                dtbl.Columns.Add("Remarks");
                dtbl.AcceptChanges();
            }          
            string Itemgrps = "";
            foreach (ListItem item in this.chkbxprodgrp.Items)
            {
                if (item.Selected)
                {
                    chkdval = true;
            ...
            if (!string.IsNullOrEmpty(txtDist.Text))
            {
                ...
                {
                    string partyname = "";
                    partyname = txtDist.Text;
                    DataRow dr = dtbl.NewRow();
                    dr["Client"] = partyname;
                    dr["ProdGrp"] = Itemgrps.Substring(0, Itemgrps.Length - 1);
                    dr["ClientID"] = hdnparty.Value; //Request.Form[ddlparty.UniqueID]; 
                    dr["ProdGrpID"] = Itemgrps.Substring(0, Itemgrps.Length - 1);
                    dr["Remarks"] = ConvPartyremarks.InnerText;
                    dtbl.Rows.Add(dr);
                    dtbl.AcceptChanges();
                    ViewState["ClientProduct"] = dtbl;
                    gdvprodgrp.DataSource = dtbl;
                    gdvprodgrp.DataBind();
                ...
                if (!chkdval)
                {
                    ...
                    lblvalidate.InnerText = "Please Select atleast one Item Group";
                    ViewState["ClientProduct"] = dtbl;
                    gdvprodgrp.DataSource = dtbl;
                    gdvprodgrp.DataBind();
                }
            ...
            {
                lblvalidate.InnerText = "Please Select Party";
              
            }
            string SelctdCity = Request.Form[ddlConvcity.UniqueID];
            string SelctdState = ddlConvstate.SelectedValue;
           // string SelctdParty = Request.Form[ddlparty.UniqueID];
            BindState();
            ddlConvstate.SelectedValue = SelctdState;
            string strQ = "select Distinct(CityID),CityName from ViewGeo VG where VG.StateId=" + SelctdState + " order by CityName";
            fillDDLDirect(ddlConvcity, strQ, "CityID", "CityName", 1);
            ...
            BindConvClient(); //ddlparty.SelectedValue = SelctdParty;
            if (ViewState["ExpCode"].ToString().ToUpper() == "CONVEYANCETRAVEL")
            {
                if (Convert.ToDecimal(RatePerKm.Text) > 0) { spntrmode.Attributes.Remove("style"); } else { spntrmode.Attributes.Add("style", "display:none"); }
            }
            this.ModalPopupExtender2.Show();
        }
       protected void gdvprodgrp_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            int index = Convert.ToInt32(e.RowIndex);
            DataTable dt = (DataTable)ViewState["ClientProduct"];
            if (ViewState["ExpCode"].ToString().ToUpper() != "CONVEYANCE")
            {
                trconv.Attributes.Remove("style");
            }
