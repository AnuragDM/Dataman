...
{
    public partial class CoupanRetailerReport : System.Web.UI.Page
    {
        ...
        private void BindZone()
        {
            try
            {
                ...
                if (Settings.Instance.RoleType == "Distributor")
                {
                    ...
                    BindDistributor(ddlZone.SelectedValue);
        ...
        private Int32 BindDistributor(string zone)
        {
            try
            {
                string str = "";
                if (Settings.Instance.RoleType == "Distributor")
                {
                    str = @"SELECT mp.PartyId,(mp.PartyName + ' - ' + ma.AreaName) AS PartyName FROM MastParty mp LEFT JOIN mastarea ma ON mp.AreaId=ma.AreaId where mp.partyid='" + Settings.Instance.DistributorID + "' and mp.partydist=1 and mp.Active=1 and mp.Zone='" + zone +"' ORDER BY mp.PartyName";
                    fillDDLDirectDropdown(ddlDistributor, str, "PartyId", "PartyName");
                }
                else
                {
                    str = @"SELECT mp.PartyId,(mp.PartyName + ' - ' + ma.AreaName) AS PartyName FROM MastParty mp LEFT JOIN mastarea ma ON mp.AreaId=ma.AreaId where mp.partydist=1 and mp.Active=1 and mp.Zone='" + zone + "' ORDER BY mp.PartyName";
        ...
        private void fillCoupan()
        {
            try
            {
                ...
                str = @"SELECT mp.Id, mp.RetailerName,mp.MobileNo,mp.Area,(SELECT count(*) FROM TransCoupan tc1 WHERE tc1.RetailerId=mp.Id) AS IssuedCoupan ,(SELECT count(*) FROM TransCoupan tc1 WHERE tc1.RetailerId IS NULL AND tc1.RetailerId=mp.Id) AS AvailableCoupons FROM TransCoupan tc LEFT JOIN MastCoupanRetailer mp ON tc.RetailerId=mp.Id WHERE tc.RetailerId IS NOT NULL and tc.SchemeId=" + ddlScheme.SelectedValue + " " + stradd + "  GROUP BY mp.Id, mp.RetailerName,mp.MobileNo,mp.Area ORDER BY mp.RetailerName";
        ...
        private void fillCoupan1()
        {
            try
            {
                ...
                str = @"SELECT mp.partyid as distributorid, mp.partyname as Distributor,mp.mobile,ma.areaname,(SELECT count(*) FROM TransCoupan tc1 WHERE tc1.RetailerId IS NULL AND tc1.DistId=mp.partyid) AS AvailableCoupons FROM TransCoupan tc LEFT JOIN Mastparty mp ON tc.DistId=mp.partyid left join mastarea ma on mp.AreaId=ma.areaid WHERE tc.Distid IS NOT NULL And tc.SchemeId=" + ddlScheme.SelectedValue + " " + stradd + "  GROUP BY mp.partyid, mp.partyname,mp.mobile,ma.areaname ORDER BY mp.partyname";
        ...
        protected void rpt_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "coupanissued")
            {
                Repeater1.Visible = true;
                Repeater1.DataSource = null;
                Repeater1.DataBind();

                string str = @"SELECT RetailerName,coupanNoWithPrefix,retailerissuedate FROM TransCoupan WHERE RetailerId =" + e.CommandArgument.ToString();
                DataTable dt = DbConnectionDAL.GetDataTable(CommandType.Text, str);
                if (dt.Rows.Count > 0)
                {
                    Repeater1.DataSource = dt;
        ...
        {
            if (e.CommandName == "coupanAvailable")
            {
                Repeater1.Visible = true;
                Repeater1.DataSource = null;
                Repeater1.DataBind();
