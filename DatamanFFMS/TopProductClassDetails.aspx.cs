...
{
    public partial class TopProductClassDetails : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                //txtfmDate.Text = Settings.dateformat(From) + " 00:00'";// System.DateTime.Now.ToShortDateString();
               // txttodate.Text = Settings.dateformat(To) + " 23:59'";// System.DateTime.Now.ToShortDateString();
        ...
        private void fillRepeter()
        {
            ...
            sql = "SELECT os.VDate,ms.SMName,mp.PartyName,mi.ItemName,convert(int,sum(os.Qty)) AS qty, convert(int,sum(os.rate)) AS rate, convert(decimal,sum(os.Qty*os.Rate)) AS amount FROM TransOrder1 os LEFT JOIN mastparty mp ON os.PartyId=mp.PartyId LEFT JOIN mastitem mi ON os.ItemId=mi.ItemId LEFT JOIN mastitemclass mc ON mi.ClassId=mc.Id LEFT JOIN mastsalesrep ms ON os.SMId=ms.SMId WHERE mc.Id='" + ViewState["item"] + "' and os.VDate>='" + ViewState["Fromdate"] + "' and os.VDate<='" + ViewState["Todate"] + "' AND mp.PartyDist=0 and os.smid in (SELECT smid FROM MastSalesRep WHERE SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + ViewState["outputStr"].ToString() + ")) and Active=1) GROUP BY os.VDate,ms.SMName,mp.PartyName,mi.ItemName,rate Union ALl SELECT os.VDate,ms.SMName,mp.PartyName,mi.ItemName,convert(int,sum(os.Qty)) AS qty, convert(int,sum(os.rate)) AS rate, convert(decimal,sum(os.Qty*os.Rate)) AS amount FROM Temp_TransOrder1 os LEFT JOIN mastparty mp ON os.PartyId=mp.PartyId LEFT JOIN mastitem mi ON os.ItemId=mi.ItemId LEFT JOIN mastitemclass mc ON mi.ClassId=mc.Id LEFT JOIN mastsalesrep ms ON os.SMId=ms.SMId WHERE mc.Id='" + ViewState["item"] + "' and os.VDate>='" + ViewState["Fromdate"] + "' and os.VDate<='" + ViewState["Todate"] + "' AND mp.PartyDist=0 and os.smid in (SELECT smid FROM MastSalesRep WHERE SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + ViewState["outputStr"].ToString() + ")) and Active=1) GROUP BY os.VDate,ms.SMName,mp.PartyName,mi.ItemName,rate";

            //sql = "SELECT convert(int,sum(os.Qty)) AS qty, convert(int,sum(os.rate)) AS rate,convert(decimal,sum(os.Qty*os.Rate)) AS amount,mp.PartyName FROM TransOrder1 os LEFT JOIN mastparty mp ON os.PartyId=mp.PartyId LEFT JOIN mastitem mi ON os.ItemId=mi.ItemId LEFT JOIN mastitemclass mc ON mi.ClassId=mc.Id WHERE mc.Id='" + item + "' and os.VDate>='" + From + "' and os.VDate<='" + To + "' AND mp.PartyDist=0 and os.smid in (SELECT smid FROM MastSalesRep WHERE SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + outputStr + ")) and Active=1)  GROUP BY mp.PartyName,rate Union ALl SELECT convert(int,sum(os.Qty)) AS qty, convert(int,sum(os.rate)) AS rate,convert(decimal,sum(os.Qty*os.Rate)) AS amount,mp.PartyName FROM Temp_TransOrder1 os LEFT JOIN mastparty mp ON os.PartyId=mp.PartyId LEFT JOIN mastitem mi ON os.ItemId=mi.ItemId LEFT JOIN mastitemclass mc ON mi.ClassId=mc.Id WHERE mc.Id='" + item + "' and os.VDate>='" + From + "' and os.VDate<='" + To + "' AND mp.PartyDist=0 and os.smid in (SELECT smid FROM MastSalesRep WHERE SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + outputStr + ")) and Active=1)  GROUP BY mp.PartyName,rate";
        ...
        private void fillRepeter1()
        {
            ...
            mastItemQry1 = @"select Id,Name from mastitemClass where Id in (" + item + ") and Active=1";  
        ...
        protected void btnExport(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=TopProductClassDetails.csv");
