...
{
    public partial class SalesTargetFormDist : System.Web.UI.Page
    {
        ...
        private void fillUserType()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                btnsave.Visible = false;
                // lblPartTypeName.Visible = false;
                GridView1.HeaderRow.Cells[1].Text = "Assign to Distributor";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                LinkButton lnkRegion = (LinkButton)e.Row.FindControl("lnkRegion");
                ...
                //if (lk != null)
                //{
                //    string s = @"select * from SalesTragetFromHO Where  PartyTypeId=" + hid.Value + " and SMID=" + Settings.Instance.SMID + " and SalesYear='" + txtcurrentyear.SelectedItem.Text + "' and Isnull(Itemtype,'Group')='" + itemgroupRadioButtonlst.SelectedValue + "' and Isnull(TargetFor,'Amount')='" + targetRadioButtonList.SelectedValue + "'";
                ...
                if (lnkRegion != null)
                {
                    ...
                    {
                        s = @"select * from SalesTargetFromHODist Where  PartyTypeId=" + hid.Value + " and AssignedByID=" + Settings.Instance.UserID + " and SalesYear='" + txtcurrentyear.SelectedItem.Text + "'  and Isnull(Itemtype,'Group')='" + itemgroupRadioButtonlst.SelectedValue + "' and Isnull(TargetFor,'Amount')='" + targetRadioButtonList.SelectedValue + "'";    
                    }
                    else
                    {
                        s = @"select * from SalesTargetFromHODist Where  PartyTypeId=" + hid.Value + " and AssignedByID=" + Settings.Instance.UserID + " and SalesYear='" + txtcurrentyear.SelectedItem.Text + "'  and Isnull(Itemtype,'Group')='" + itemgroupRadioButtonlst.SelectedValue + "' and Isnull(TargetFor,'Amount')='" + targetRadioButtonList.SelectedValue + "' and Unit='" + ddlunit.SelectedItem.Text + "'";
                    }
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "Region")
            {
                ...
                //else
                //{
                //    if (targetRadioButtonList.SelectedItem.Text == "Quantity")
                //    {
                //        System.Web.UI.ScriptManager.RegisterStartupScript(this, GetType(), "Error", "errormessage('Please select unit');", true);
                //        return;
                //    }
                ...
                //DataTable PartyDT = new DataTable();
                //PartyDT = DbConnectionDAL.GetDataTable(CommandType.Text, str1);
                //if (itemgroupRadioButtonlst.SelectedItem.Text == "Group")
                //{
                //    lblPartTypeName.Text = "Product Group " + PartyDT.Rows[0][0].ToString();
                //}
                //else
            ...
            //    lblPartTypeName.Text = PartyDT.Rows[0][0].ToString();
                BindArea();
                lblPartTypeID.Text = e.CommandArgument.ToString();
                ...
                //    try
                //    {
                //        DataView dv = new DataView(dtDist);
                //        // dv.RowFilter = "SMId <>" + Settings.Instance.SMID;
                //        if (dv.Table.Rows.Count > 0)
                //        {
                //            GridView2.DataSource = dv;
            ...
            else if (e.CommandName == "State")
            {
                ...
                {
                    lblPartTypeName.Text = PartyDT.Rows[0][0].ToString();
                    d = Settings.UnderUsers1(Settings.Instance.SMID);
                    if (d.Rows.Count > 0)
                    {
                        try
                        {
        ...
        {
            string docID = Settings.GetDocID("STARD", DateTime.Now);
            Settings.SetDocID("STARD", docID);
            try
            {
                string s="";
                ...
                for (int i = 0; i < GridView2.Rows.Count; i++)
                {
                    ...
                    int RetSave = MT.Inserttargetfordist(docID, Settings.DMInt32(lblPartTypeID.Text), Settings.DMDecimal(jan.Text), Settings.DMDecimal(feb.Text), Settings.DMDecimal(mar.Text), Settings.DMDecimal(apr.Text), Settings.DMDecimal(may.Text), Settings.DMDecimal(jun.Text), Settings.DMDecimal(jul.Text), Settings.DMDecimal(aug.Text), Settings.DMDecimal(spt.Text), Settings.DMDecimal(oct.Text), Settings.DMDecimal(nov.Text), Settings.DMDecimal(dec.Text), Settings.DMInt32(hidPartyId.Value), i + 1, Settings.DMInt32(Settings.Instance.UserID), txtcurrentyear.SelectedItem.Text, itemgroupRadioButtonlst.SelectedValue, targetRadioButtonList.SelectedValue, ddlunit.SelectedItem.Text);
                }
        ...
        {
            Response.Redirect("~/SalesTargetFormDist.aspx");
        }
        ...
        protected void GridView2_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (targetRadioButtonList.SelectedValue == "Amount")
                {
                    srty = "select * from SalesTargetFromHODist where DistId=" + Settings.DMInt32(PartyId.Value) + " and PartyTypeId=" + lblPartTypeID.Text + " and SalesYear='" + txtcurrentyear.SelectedItem.Text + "' and Isnull(Itemtype,'Group')='" + itemgroupRadioButtonlst.SelectedValue + "' and Isnull(TargetFor,'Amount')='" + targetRadioButtonList.SelectedValue + "'";
                }
                else
                {
                    srty = "select * from SalesTargetFromHODist where DistId=" + Settings.DMInt32(PartyId.Value) + " and PartyTypeId=" + lblPartTypeID.Text + " and SalesYear='" + txtcurrentyear.SelectedItem.Text + "' and Isnull(Itemtype,'Group')='" + itemgroupRadioButtonlst.SelectedValue + "' and Isnull(TargetFor,'Amount')='" + targetRadioButtonList.SelectedValue + "' and Unit='" + ddlunit.SelectedItem.Text + "'";
                }
        ...
        protected void GridView3_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (lk != null)
                {
                    string s = @"select * from SalesTargetFromHODist Where  PartyTypeId=" + lblPartTypeID.Text + " and DistId=" + hid.Value + " and SalesYear='" + txtcurrentyear.SelectedItem.Text + "' and Isnull(Itemtype,'Group')='" + itemgroupRadioButtonlst.SelectedValue + "' and Isnull(TargetFor,'Amount')='" + targetRadioButtonList.SelectedValue + "'";
                    DataTable dtr = new DataTable();
                    dtr = DbConnectionDAL.GetDataTable(CommandType.Text, s);
                    if (dtr.Rows.Count > 0)
                    {
                ...
                if (lblAllocatetoTeam != null)
                {
                    ...
                    string s = @"select * from SalesTragetFromHO Where  PartyTypeId=" + lblPartTypeID.Text + " and AssignedByID=" + hidUid.Value + " and SalesYear='" + txtcurrentyear.SelectedItem.Text + "' and Isnull(Itemtype,'Group')='" + itemgroupRadioButtonlst.SelectedValue + "' and Isnull(TargetFor,'Amount')='" + targetRadioButtonList.SelectedValue + "'";
                    DataTable dtr = new DataTable();
                    dtr = DbConnectionDAL.GetDataTable(CommandType.Text, s);
                    if (dtr.Rows.Count > 0)
                    {
        ...
        protected void ListArea_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //string str1 = "select ItemName as Name,ItemId as Id from MastItem where ItemType='MATERIALGROUP' and  ItemId=" + e.CommandArgument.ToString();
            string str1 = "select ItemName as Name,ItemId as Id from MastItem where ItemId=" + lblPartTypeID.Text ;
            DataTable PartyDT = new DataTable();
            PartyDT = DbConnectionDAL.GetDataTable(CommandType.Text, str1);
           ...
           // lblPartTypeID.Text = e.CommandArgument.ToString();
            ...
            if (matareaStr != "")
            {
                distqry = @"select PartyId,PartyName from MastParty where  areaid in (" + matareaStr + ") and PartyDist=1 and Active=1 order by PartyName";
            ...
            else
            {
                ...
                GridView2.DataBind();
                btnsave.Visible = false;
                System.Web.UI.ScriptManager.RegisterStartupScript(this, GetType(), "Error", "errormessage('Please select Area');", true);
                return;
            }
