...
{
    public partial class MeetType : System.Web.UI.Page
    {
        ...
        {
            string str = @"  select *,case UserType when 1 then 'Retailer'  when 2 then 'EndUser'    when 3 then 'Dealer' end as UserType1 from MastMeetType";
        ...
        private void FillDeptControls(int ProspectId)
        {
            try
            {
                ...
                //string str = @"select * from MastMeetType  where Id=" + ProspectId;
                string str = @"select Name,userType,Invition from MastMeetType  where Id=" + ProspectId;
                ...
                if (deptValueDt.Rows.Count > 0)
                {
                    ...
                    ddlUserType.SelectedValue = deptValueDt.Rows[0]["userType"].ToString();
                    chkinvitation.Checked =Convert.ToBoolean(deptValueDt.Rows[0]["Invition"].ToString());
        ...
        {
            string s = "select count(*) from MastMeetType where Name='"+txtName.Text+"'";
        ...
        {
            string s = "select count(*) from MastMeetType where Name='" + txtName.Text+"' and Id !='"+Convert.ToInt32(ViewState["VisId"])+"'";
            int exists = Convert.ToInt32(DbConnectionDAL.GetScalarValue(CommandType.Text, s));
            return exists;
        ...
        {
            int retsave = dp.Insert(txtName.Text,Convert.ToInt32(ddlUserType.SelectedValue),Convert.ToBoolean(chkinvitation.Checked));
            if (retsave != 0)
            {
                 ClearControls();
                 btnDelete.Visible = false;
        ...
        private void ClearControls()
        {
            ...
            chkinvitation.Checked = true;
            btnDelete.Visible = false;
        }

        private void UpdateRecord()
        {
            int retsave = dp.Update(Convert.ToInt32(ViewState["VisId"]), txtName.Text, Convert.ToInt32(ddlUserType.SelectedValue), Convert.ToBoolean(chkinvitation.Checked));
            if (retsave == 1)
            {
        ...
        {
            Response.Redirect("~/MeetType.aspx");
        }
