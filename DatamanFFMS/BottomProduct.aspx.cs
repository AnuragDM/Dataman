...
{
    public partial class BottomProduct : System.Web.UI.Page
    {
        string roleType = "";
        int cnt = 0;
        string orderType = "";
        ...
        protected void btnGo_Click(object sender, EventArgs e)
        {
            ...
            if (smIDStr1 != "")
            {
                ...
                string query = "SELECT TOP " + txt_noofrecords.Text + " ProductGroup,itemid,Product,convert(int,sum(Qty)) [Qty],convert(decimal, sum(Amount)) [Amount] FROM(SELECT pg.ItemName [ProductGroup],i.itemid,i.ItemName [Product],convert(int,(os.Qty)) [Qty],convert(decimal, (os.Qty * os.Rate)) [Amount] From  TransOrder1 os left JOIN MastItem i ON os.ItemId = i.ItemId ";
                query = query + "left JOIN MastItem pg ON pg.ItemId = i.Underid " + Qrychk + " " + QueryMatGrp + " Union All SELECT pg.ItemName [ProductGroup],i.itemid,i.ItemName [Product],convert(int,(os.Qty)) [Qty],convert(decimal, (os.Qty * os.Rate)) [Amount] From Temp_TransOrder1 os left JOIN MastItem i ON os.ItemId = i.ItemId ";
        ...
        {
            Response.Redirect("~/BottomProduct.aspx");
        }
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=BottomProduct.csv");
            string headertext = "Product Group".TrimStart('"').TrimEnd('"') + "," + "Product".TrimStart('"').TrimEnd('"') + "," + "Qty".TrimStart('"').TrimEnd('"') + "," + "Amount".TrimStart('"').TrimEnd('"');
        ...
        {
            Response.Redirect("~/BottomProductDetails.aspx");
        }
        ...
        protected void Topsaleproduct_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "select")
            {
                ...
                Response.Redirect("BottomProductDetails.aspx?itemid=" + hdnitemid.Value + "&FromDate=" + ViewState["Fromdate"] + "&ToDate=" + ViewState["Todate"] + "&smidstr='" + ViewState["smIDStr1"].ToString() + "'");
            }
        }
    }
}