...
{
    public partial class DistInvoiceReport : System.Web.UI.Page
    {
        string roleType = "";
        int cnt = 0;
...
//                           ON mp.PartyId=tdv.DistId left join MastResCentre mrc on mrc.ResCenId=tdv1.LocationID where " + Qrychk + " AND mp.PartyDist=1 and tdv.DistId in  (" + Distid + ")  group by tdv.DistInvDocId,mp.SyncId,mp.PartyName,tdv.VDate, mrc.ResCenName ORDER BY tdv.VDate desc ";
            ...
            string query = @"select tdv1.DistInvDocId,mp.SyncId AS PartyId,mp.PartyName,Convert(varchar(15),CONVERT(date,tdv1.VDate,103),106) AS VDate,Sum(tdv1.Amount) as Amount,
                           mrc.ResCenName as BranchName from TransDistInv1 tdv1 LEFT JOIN mastparty mp ON mp.PartyId=tdv1.DistId left join MastResCentre mrc on
                           mrc.ResCenId=tdv1.LocationID where " + Qrychk + " AND mp.PartyDist=1 and tdv1.DistId in  (" + Distid + ")  group by tdv1.DistInvDocId,mp.SyncId,mp.PartyName,tdv1.VDate, mrc.ResCenName ORDER BY tdv1.VDate desc ";
