...
{
    public partial class VarianceReportL1 : System.Web.UI.Page
    {
        ...
        {
            Response.Redirect("~/VarianceReportL1.aspx");
        }
        ...
        public void BindGrid()
        {
            try
            {
                ...
                mSRepCode = @"select SMId,SMName,SyncId from MastSalesRep where SMId in (SELECT ms.smid from mastsalesrep ms LEFT JOIN MastRole mr ON ms.RoleId= mr.RoleId WHERE ms.SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + smIDStr1 + ")) AND mr.RoleType IN ('AreaIncharge')) and Active=1 order by SMId";
                ...
                string No_Days = "select NoOfDays,SMId,FromDate ,ToDate as NoDays,AppStatus,LeaveString from TransLeaveRequest lr where lr.SMId in (SELECT ms.smid from mastsalesrep ms LEFT JOIN MastRole mr ON ms.RoleId= mr.RoleId WHERE ms.SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + smIDStr1 + ")) AND mr.RoleType IN ('AreaIncharge')) and (lr.FromDate between '" + Settings.dateformat(mDate1.ToString()) + " 00:00' and '" + Settings.dateformat(mDate2.ToString()) + " 23:59' OR lr.ToDate between '" + Settings.dateformat(mDate1.ToString()) + " 00:00' and '" + Settings.dateformat(mDate2.ToString()) + " 23:59') and lr.appstatus in ('Approve','Pending') order by smid,FromDate";
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=VarianceReportL1.csv");
            Response.Write(sb.ToString());
            Response.End();

            //Response.Clear();
            //Response.Buffer = true;
            //Response.AddHeader("content-disposition", "attachment;filename=VarianceReportL1.xls");
            //Response.Charset = "";
