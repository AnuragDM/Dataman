...
{
    public partial class InvoiceSAPDetailsForRetailer : System.Web.UI.Page
    {
        BAL.PurchaseOrder.PurchaseOrder dp = new BAL.PurchaseOrder.PurchaseOrder();
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Request.QueryString["RetInvDocId"] != null)
            {
                string DistInvDocId = Convert.ToString(Request.QueryString["RetInvDocId"]);
        ...
        private void LoadDetails(string DistInvDocId)
        {
            ...
            try
            {              
                string str = @"SELECT T1.BillAmount,T1.PartyID,ISnull(T2.LocationID,0) as LocationID,REPLACE(CONVERT(CHAR(11), T1.vdate, 106),' ','/') AS VDate FROM TransRetailerInv AS T1
                                    LEFT JOIN TransRetailerInv1 AS T2
                                    ON T1.RetInvId=T2.RetInvId WHERE T1.RetInvDocId='" + DistInvDocId + "'";


                DataTable obj = new DataTable();
                obj = DbConnectionDAL.GetDataTable(CommandType.Text, str);
                BillAmount=Convert.ToDecimal(obj.Rows[0]["BillAmount"].ToString());
                BindDistDetails(Convert.ToInt32(obj.Rows[0]["PartyID"]));                
                ...
                string str1 = @"SELECT isnull(T.OrderNo,'') AS OrderNo,Isnull(T.LocationID,0) as LocationID FROM TransRetailerInv1 AS T WHERE T.RetInvDocId='" + DistInvDocId + "'";
                ...
                string tblstr = "<table><tbody>";
                 str = @"SELECT Description, Amount FROM TransRetailerInv2 WHERE RetInvDocId='" + DistInvDocId + "'";
                ...
                {
                    tblstr = tblstr + "<tr><td> <label for='exampleInputEmail1'>" + obj.Rows[i]["Description"].ToString() + " </label> </td><td>:</td><td style='text-align: right;'> <label for='exampleInputEmail1'>" + obj.Rows[i]["Amount"].ToString() + " </label> </td></tr>";
                }

                tblstr = tblstr + "<tr><td> <label for='exampleInputEmail1'>Total</label></td><td>:</td><td style='text-align: right;'> <label for='exampleInputEmail1'>" + BillAmount + " </label></td></tr></tbody></table>";

                tbl.InnerHtml=tblstr;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (compcode == "BAF")
                {
                    str = @"SELECT '' AS ItemID,ItemName AS ItemName,'' as StdPack,'' as Qty,   ''  AS Remarks,'' as Unit,'' as Rate,''  AS Amount,'' as PriceGroup 
                                FROM TransRetailerInv1 AS T1
                                WHERE T1.RetInvDocId='" + DistInvDocId + "' and T1.LocationID=" + LocationID + "";
                }
                else
                {
                    str = @"SELECT T.ItemId AS ItemID,T.ItemName AS ItemName,T.StdPack,T1.Qty,
                                T1.ItemRemarks AS Remarks,T1.Unit,T1.Rate,T1.Amount AS Amount,T.PriceGroup
                                FROM MastItem AS T
                                Right JOIN TransRetailerInv1 AS T1
