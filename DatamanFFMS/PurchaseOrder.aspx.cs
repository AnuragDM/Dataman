...
namespace FFMS
{
    ...
    {  
        ...
        private void FillPOData(int POID)
        {
            ...
            if(dt1!=null && dt1.Rows.Count>0)
            {
                ...
                for(int i=0;i<dt1.Rows.Count;i++)
                {
                    ...
                    dtItem.Rows[dtItem.Rows.Count - 1]["Qty"] = Math.Round((Convert.ToDecimal(dt1.Rows[i]["Qty"])), 0);
                    ...
                    dtItem.Rows[dtItem.Rows.Count - 1]["StdPack"] = Math.Round((Convert.ToDecimal(dt1.Rows[i]["StdPack"])), 0); 
        ...
        private void fillRepeter()
        {
            ...
            if (RoleType.ToUpper() == "Admin".ToUpper())
            {
                str = @"SELECT T1.POrdId,T1.VDate,T1.PODocId,T2.PartyName,T3.AreaName,
                            ...
                            where T2.Active=1   " +
                                  @" " + mainQry + " and (T1.OrderStatus='P') ORDER BY T1.VDate desc";
            }
            else
            {
                str = @"SELECT T1.POrdId,T1.VDate,T1.PODocId,T2.PartyName,T3.AreaName,
                            ...
                            LEFT JOIN MastArea AS T3
                            ON T2.CityId = T3.AreaId                           
                            where T2.Active=1 and T2.CityId in (select AreaId from mastarea where areaid in 
							(select underid from mastarea where areaId  in 
							(select linkcode from mastlink where primtable='SALESPERSON' and LinkTable='AREA' and PrimCode=" +Convert.ToInt32(Settings.Instance.SMID)+"))"+ 
							" and areatype='City' and Active=1) " +
                                  @" " + mainQry + " and (T1.OrderStatus='P') ORDER BY T1.VDate desc";
            }
            ...
            if (txtDist.Text != "")
            {
                ...
                if (dt != null && dt.Rows.Count>0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // 04/dec/2016
                            SendEmailMessage(Convert.ToInt32(ViewState["POID"]), Currdt, docID, Settings.Instance.UserID, Settings.Instance.SMID,
                          hfCustomerId.Value, TransPorterID, DispName.Text, DispAdd1.Text, DispAdd2.Text,
                          txtCountry.Text.Trim(), txtState.Text.Trim(), ddlCity.SelectedItem.Value, Pin.Text,
            ...
            string PType, int ProjectID, decimal decAmtTotal, int CityID, string TransPorter, int SchemeID)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                string strVar = @"SELECT * FROM MastEmailVariable WHERE EmailKey='EditOrderEmail'";
                DataTable dtVar = DbConnectionDAL.GetDataTable(CommandType.Text, strVar);
                string strEmailTemplate = @"SELECT * FROM EmailTemplate WHERE EmialKey='EditOrderEmail'";
        ...
        {
            //string str = "select * FROM MastParty where (PartyName like '%" + prefixText + "%') and PartyDist=1 and Active=1 ORDER BY PartyName";
            ...
            //                           " and T1.CityId in (select AreaId from mastarea where areaid in (select areaid from MastAreaGrp where MainGrp " +
            //                   " in (select linkcode from mastlink where primtable='SALESPERSON' and LinkTable='AREA' and PrimCode=" + Convert.ToInt32(Settings.Instance.SMID) + ")) and areatype='City' and Active=1) ORDER BY PartyName";
                  ...
                  + Settings.Instance.RoleID + "").ToString();
                    if (RoleType.ToUpper() == "Admin".ToUpper())
                    {
                        str = @"SELECT T1.POrdId,T1.VDate,T1.PODocId,T2.PartyName,T3.AreaName,
                            ...
                            ON T2.CityId = T3.AreaId                           
                            where T2.Active=1 " +
                        @" " + mainQry + " and (T1.DistId=" + DistID + " OR " + DistID + "=0) and (T1.OrderStatus='" + StatusVal + "' OR " + StatusVal + "='0') ORDER BY T1.VDate desc";
                    }
                    else
                    {
                        str = @"SELECT T1.POrdId,T1.VDate,T1.PODocId,T2.PartyName,T3.AreaName,
                ...
                else
                {
                  ...
                  + Settings.Instance.RoleID + "").ToString();
                         if (RoleType.ToUpper() == "Admin".ToUpper())
                         {
                            ...
                            ON T2.CityId = T3.AreaId                           
                            where T2.Active=1 " +
                          @" " + mainQry + " and (T1.DistId=" + DistID + " OR " + DistID + "=0) and (T1.OrderStatus='" + StatusVal + "') ORDER BY T1.VDate desc";
                         }
                         else
                         {
                             str = @"SELECT T1.POrdId,T1.VDate,T1.PODocId,T2.PartyName,T3.AreaName,
        ...
        protected void CancelOrder_Click(object sender, EventArgs e)
        {
            ...
            if (confirmValue == "Yes")
            {
                ...
                {
                    Label lblAmtTotal = (Label)gvDetails.FooterRow.FindControl("lblAmtTotal");
                    SendEmailMessage_CancelOrder(Convert.ToInt32(ViewState["POIDDel"]), Convert.ToDateTime(lblDate.Text.Trim()), lblOrderSt.Text.Trim(), Settings.Instance.UserID, Settings.Instance.SMID,
                          hfCustomerId.Value, 0, DispName.Text, DispAdd1.Text, DispAdd2.Text,
                           txtCountry.Text.Trim(), txtState.Text.Trim(), ddlCity.SelectedItem.Value, Pin.Text,
        ...
        private void SendEmailMessage_CancelOrder(int POID, DateTime Currdt, string docID, string UserID, string SMID, string CustID,
          int TransPorterID, string DispName, string DispAdd1, string DispAdd2, string Country, string State,
          ...
          string PType, int ProjectID, decimal decAmtTotal, int CityID, string TransPorter, int SchemeID)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                string strVar = @"SELECT * FROM MastEmailVariable WHERE EmailKey='CompanyCancelled'";
                DataTable dtVar = DbConnectionDAL.GetDataTable(CommandType.Text, strVar);
                string strEmailTemplate = @"SELECT * FROM EmailTemplate WHERE EmialKey='CompanyCancelled'";
                DataTable dtEmailTemplate = DbConnectionDAL.GetDataTable(CommandType.Text, strEmailTemplate);
