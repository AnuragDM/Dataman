...
{
    public partial class DSRVistEntry : System.Web.UI.Page
    {
         int msg = 0;
         int uid = 0;
         int smID = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                btnSave.CssClass = "btn btn-primary";
                btnNext.Enabled = Convert.ToBoolean(SplitPerm[2]);
               // btnNext.Enabled = Settings.Instance.CheckAddPermission(pageName, Convert.ToString(Session["user_name"]));
                btnNext.CssClass = "btn btn-primary";
            }
        ...
        protected void btnFind_Click(object sender, EventArgs e)
        {
            ...
            rpt.DataSource = null;
            rpt.DataBind();
            txtmDate.Text = DateTime.Parse(Settings.GetUTCTime().ToShortDateString()).AddMonths(-1).ToString("dd/MMM/yyyy");
            //txttodate.Text = DateTime.Parse(DateTime.Now.ToShortDateString()).ToString("dd/MMM/yyyy"); 
            txttodate.Text = DateTime.Parse(Settings.GetUTCTime().ToShortDateString()).ToString("dd/MMM/yyyy");
            btnSave.Text = "Save";
                ...
                if (Datecompare())
                {
                    ...
                    else
                    {
                        if (checkDateForUserExists(Convert.ToDateTime(txtVisitDate.Text)) == 0)
                        {
                            ...
                            if (checkDSRAllowdays(strdate) != 0)
                            //{ System.Web.UI.ScriptManager.RegisterStartupScript(this, GetType(), "errormessage", "errormessage('you can not enter new dsr because your previous date's dsr entry not locked or contact crm admin.');", true); }
                             { System.Web.UI.ScriptManager.RegisterStartupScript(this, GetType(), "errormessage", "errormessage('you can not enter new dsr because your previous date dsr entry not locked or contact crm admin.');", true); }
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if (retsave != 0)
                                    {
                                        ...
                                        hdnCityId.Value = "";
                                        hdnCityname.Value = "";
                                        Response.Redirect("DistributorDashboardLevel3.aspx?CityID=" + CityStr1 + "&VisitID=" + VisitID + "&DistID=" + 0);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Response.Redirect("DistributorDashboardLevel3.aspx?CityID=" + hdncitylock.Value + "&VisitID=" + VisitID + "&DistID=" + 0);
                                }
                                //           Response.Redirect("DistributorDashboardLevel3.aspx?CityID=" + ddlcity.SelectedValue + "&VisitID=" + VisitID + "&DistID=" + 0);
                            }
                        ...
                        else
                        {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (retsave != 0)
                                        {
                                            ...
                                            hdnCityname.Value = "";
                                            // Response.Redirect("DistributorDashboardLevel3.aspx?CityID=" + ddlcity.SelectedValue + "&VisitID=" + VisitID + "&DistID=" + 0);
                                            Response.Redirect("DistributerPartyList.aspx?VisitID=" + VisitID + "&CityID=" + CityStr1 + "&Level=" + 3 + "&PageL3=Secondary");
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        Response.Redirect("DistributerPartyList.aspx?VisitID=" + VisitID + "&CityID=" + hdncitylock.Value + "&Level=" + 3 + "&PageL3=Secondary");
                                    }
                                    //Response.Redirect("DistributerPartyList.aspx?VisitID=" + VisitID + "&CityID=" + ddlcity.SelectedValue + "&Level=" + 3 + "&PageL3=Secondary");
                                }
...
SELECT [VisId]  ,[FVDocId]   ,[VDate]   ,[UserID]   ,[SMId]   ,[PartyId]   ,[Remarks]   ,[AreaId]   ,[Nextvisit]   ,[ReasonID],[VisitTime],android_id,created_date,Longitude,Latitude,Lat_long_datetime,address FROM [TransFailedVisit] where VisId=" + VisitID;
                DbConnectionDAL.ExecuteNonQuery(CommandType.Text, str);
...
SELECT [VisId] ,[DocId]  ,[VDate] ,[UserId] ,[PartyId]  ,[Item]  ,[Qty]  ,[Rate],[SMID],[CompName],[ImgUrl],[Remarks],[Discount],[BrandActivity],[MeetActivity],[RoadShow],[Scheme/offers],[OtherGeneralInfo],[OtherActivity],android_id,created_date,Longitude,Latitude,Lat_long_datetime,address FROM [TransCompetitor] where VisId=" + VisitID;
                DbConnectionDAL.ExecuteNonQuery(CommandType.Text, str);
...
SELECT [UserId],[VisId] ,[Sno] ,[VDate] ,[cityId] ,[SMId] ,[DistId] ,[areaId] ,[remarkDist] ,[remarkArea]  ,[remarkL1] ,[L1UserId],[DissType],[NextVisitDate],[NextVisitTime],[SpentfrTime],[SpentToTime],[ImgUrl],android_id,created_date,DiscDocid,Longitude,Latitude,Lat_long_datetime,address FROM [TransVisitDist] where [VisId]=" + VisitID;
                DbConnectionDAL.ExecuteNonQuery(CommandType.Text, str);
