...
{
    public partial class ProductiveDistributor : System.Web.UI.Page
    {
        string roleType = "";
        int cnt = 0;
        ...
        protected void btnGo_Click(object sender, EventArgs e)
        {
           ...
           // foreach (ListItem Beat in Lstbeatbox.Items)
            ...
            if (smIDStr1 != "")
            {
                ...
                string query = @"SELECT distinct tp.PODocId, mp.PartyName AS Distributor,mp.Address1,mp.Mobile,ms.SMName FROM TransPurchOrder tp LEFT JOIN mastparty mp ON tp.DistId=mp.PartyId LEFT JOIN mastsalesrep ms ON tp.SMId=ms.SMID WHERE tp.VDate>='" + Settings.dateformat(txtfmDate.Text) + " 00:00' and tp.VDate<='" + Settings.dateformat(txttodate.Text) + " 23:59' and tp.smid in (SELECT smid FROM MastSalesRep WHERE SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + smIDStr1 + ")) and Active=1) GROUP BY tp.PODocId,mp.PartyName,mp.Address1,mp.Mobile,ms.SMName";
...
//              " union all select distinct p.PartyId,p.PartyName as Name, max(p.Address1) AS Address,max(p.Mobile) as Mobile,max(b.AreaName) AS beat,max(ms.SMName) AS [By],sum(isnull(om.OrderAmount,0)) AS Business,sum(isnull(om.OrderAmount,0)) AS Pendency,max(p.Created_Date) as Date from TransOrder om inner join MastParty p on p.PartyId=om.PartyId left JOIN MASTAREA b on b.AreaId=p.BeatId LEFT JOIN mastsalesrep ms ON ms.UserId=p.Created_User_id WHERE p.PartyDist=0 " + Qrychk + " " + Qty2 + " " + beat_Filter + " group by p.PartyId,p.PartyName) a Group by a.PartyId,a.Name,a.Address,a.Mobile Order by Business desc,MAX( a.Date) DESC,name";
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=ProductiveDistributor.csv");
            ...
            foreach (RepeaterItem item in rptnewrtl.Items)
            {
                ...
                dr["Distributor"] = lblName.Text.ToString().Replace("\n", "").Replace("\r", "").Replace(',', ' ');
