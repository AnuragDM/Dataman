...
{
    public partial class SalesPersonL2UC : System.Web.UI.UserControl
    {
        int smLogID = 0;
        DataTable Dt2;
        DataTable smDT;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            { //Ankita - 09/may/2016- (For Optimization)
             //   GetSMId(Settings.Instance.UserID);
                ...
                tourMonthDDL.SelectedValue = System.DateTime.Now.Month.ToString();
                tourYearDDL.SelectedValue = System.DateTime.Now.Year.ToString();
        ...
        private void BindDDLMonth()
        {
            try
            {
                for (int month = 1; month <= 12; month++)
                {
                    ...
                    tourMonthDDL.Items.Add(new ListItem(monthName.Substring(0, 3), (month.ToString().PadLeft(2, '0')).TrimStart('0')));
                }
                for (int i = System.DateTime.Now.Year - 10; i <= (System.DateTime.Now.Year); i++)
                {
                    ...
                    tourYearDDL.Items.Add(new ListItem(i.ToString()));
                }
            }
            catch (Exception ex)
            {
...
//                " union ALL select mp.PartyName,ms.SMName,MAX(mig.ItemName) AS MaterialGroup,mic.Name AS ItemClass,0 as [Amount],0 as preAmt,convert(numeric(18, 2), sum(Amount/6)) as MonthAvg from TransDistInv1 tdi left join MastItem mi on tdi.ItemId=mi.ItemId left JOIN mastitem mig on mi.Underid=mig.ItemId left join MastItemClass mic on mic.Id=mi.ClassId left join MastItemSegment mis on mis.Id=mi.SegmentId left join MastParty MP on mp.PartyId=tdi.DistId  and mp.PartyDist=1 left join MastArea on MastArea.AreaId=MP.CityId left join transdistinv tdin on tdin.distinvdocid=tdi.distinvdocid LEFT JOIN mastsalesrep ms ON ms.SMId = tdin.SMID WHERE  mig.ItemType='materialGroup' and tdin.smid in (select smid from mastsalesrepgrp where maingrp = " + Settings.Instance.SMID + ") AND tdi.VDate between  '" + Settings.dateformat(dt_1.ToString()) + " 00:00'  and '" + Settings.dateformat(predt1.ToString()) + " 23:59' GROUP BY PartyName,mig.ItemName,mic.Name,mis.Name,ms.SMName )a group BY a.PartyName,a.materialGroup,a.itemClass,a.SMName";
               ...
               " union ALL select mp.PartyName,ms.SMName,MAX(mig.ItemName) AS MaterialGroup,mic.Name AS ItemClass,0 as [Amount],0 as preAmt,convert(numeric(18, 2), sum(Amount/6)) as MonthAvg from TransDistInv1 tdi left join MastItem mi on tdi.ItemId=mi.ItemId left JOIN mastitem mig on mi.Underid=mig.ItemId left join MastItemClass mic on mic.Id=mi.ClassId left join MastItemSegment mis on mis.Id=mi.SegmentId left join MastParty MP on mp.PartyId=tdi.DistId  and mp.PartyDist=1 left join MastArea on MastArea.AreaId=MP.CityId left join transdistinv tdin on tdin.distinvdocid=tdi.distinvdocid LEFT JOIN mastsalesrep ms ON ms.SMId = tdin.SMID WHERE  mig.ItemType='materialGroup' and tdin.Distid in (" + diststr + ") AND tdi.VDate between  '" + Settings.dateformat(dt_1.ToString()) + " 00:00'  and '" + Settings.dateformat(predt1.ToString()) + " 23:59' GROUP BY PartyName,mig.ItemName,mic.Name,mis.Name,ms.SMName )a group BY a.PartyName,a.materialGroup,a.itemClass,a.SMName";
                ...
                if (primsaledt.Rows.Count > 0)
                {
                    ...
                    TourJQX.Style.Add("display", "none");
        ...
        protected void TourPlan_Click(object sender, EventArgs e)
        {
            try
            {
                tourYearDDL.SelectedValue = (Settings.GetUTCTime().Year).ToString();
                tourMonthDDL.SelectedValue = (Settings.GetUTCTime().Month).ToString();
                DateTime date1 = Convert.ToDateTime(Settings.GetUTCTime().Day + "/" + Settings.GetUTCTime().Month + "/" + Settings.GetUTCTime().Year);
                DateTime date2 = date1.AddDays(30);

                //Added
                DateTime date3;
                DateTime date4;
                if(Settings.GetUTCTime().Month.ToString()=="1")
                {
                    date3 = Convert.ToDateTime("1/" + (12) + "/" + (Settings.GetUTCTime().Year-1));
                    date4 = Convert.ToDateTime("31/" + (12) + "/" + (Settings.GetUTCTime().Year - 1));
                }
                    //End
                else
                {
                    date3 = Convert.ToDateTime("1/" + (Settings.GetUTCTime().Month - 1) + "/" + Settings.GetUTCTime().Year);
                    date4 = Convert.ToDateTime(DateTime.DaysInMonth(Convert.ToInt32(Settings.GetUTCTime().Year), Convert.ToInt32(Settings.GetUTCTime().Month) - 1) + "/" + (Settings.GetUTCTime().Month - 1) + "/" + Settings.GetUTCTime().Year);
                }



                string tourQuery = @" select TransTourPlan.VDate,TransTourPlan.MCityName AS Cityname,SMName,isnull((select sum(Qty* rate) from TransDistInv1  
                left join MastParty on MastParty.PartyId=TransDistInv1.DistId left join MastArea on MastArea.AreaId=MastParty.CityId WHERE TransDistInv1.VDate BETWEEN '" + Settings.dateformat(date3.ToString()) + "' and '" + Settings.dateformat(date4.ToString()) + "' AND MastArea.AreaId=TransTourPlan.CityId),0) [Sale] from TransTourPlan left join MastArea ma1 on ma1.AreaId=TransTourPlan.CityId left join MastSalesRep msr on msr.SMId=TransTourPlan.SMId where TransTourPlan.SMId IN (SELECT smid FROM   mastsalesrepgrp WHERE  maingrp IN ( " + Settings.Instance.SMID + " ) AND level >= (SELECT DISTINCT level FROM   mastsalesrepgrp WHERE  maingrp IN ( " + Settings.Instance.SMID + " ))) and year(TransTourPlan.VDate)='" + Settings.GetUTCTime().Year + "' and month(TransTourPlan.VDate)='" + Settings.GetUTCTime().Month + "' and TransTourPlan.AppStatus='Approve' group by transtourplan.CityId,transtourplan.VDate,TransTourPlan.AccDistributor,TransTourPlan.SMId,ma1.AreaName,SMName,TransTourPlan.MCityName Order by TransTourPlan.VDate desc";

                DataTable tourdtNew = DbConnectionDAL.GetDataTable(CommandType.Text, tourQuery);
                DataView dv = new DataView(tourdtNew);
                //        dv.RowFilter = "VDate='"+Settings.GetUTCTime().ToShortDateString()+"'";
                if (tourdtNew.Rows.Count > 0)
                {
                    reportcontainer.Style.Add("display", "block");
                    TourJQX.Style.Add("display", "block");
                    LeaveJQX.Style.Add("display", "none");
                    DSRJQX.Style.Add("display", "none");
                    DistJQX.Style.Add("display", "none");
                    PrimaryJQX.Style.Add("display", "none");
...
//                select AreaId from MastArea where AreaId in (select UnderId from MastArea where AreaId in (SELECT linkcode FROM mastlink WHERE primtable = 'SALESPERSON' AND linktable = 'AREA' AND primcode = " + Settings.Instance.SMID + "))) group by DistId,PartyName having (sum(AmtDr)-sum(AmtCr))>0 order by Balance desc";
        ...
        private void BindSalePersonDDl_SecSale()
        {
            try
            {
                DataTable dt = Settings.UnderUsers(Settings.Instance.SMID);
                ...
                if (dv.ToTable().Rows.Count > 0)
                {
                    //lstSecondarySales.DataSource = dv.ToTable();
                    //lstSecondarySales.DataTextField = "SMName";
                    //lstSecondarySales.DataValueField = "SMId";
                    //lstSecondarySales.DataBind();
                }
                //    DdlSalesPerson.Items.Insert(0, new ListItem("--Please select--"));
            }
            catch (Exception ex)
        ...
        protected void btnGoSP_Click(object sender, EventArgs e)
        {
            ...
            string expenseqry = @"SELECT sum(ED.billamount) AS billamount,ED.billdate AS BillDate,max(MET.Name) AS ExpenseName,sum(ED.claimamount) AS claimamount,
                                            Sum(Ed.ApprovedAmount) AS ApprovedAmount,msr.SMName FROM expensedetails ED INNER JOIN mastexpensetype MET 
                                                    ON ED.expensetypeid = MET.id Inner JOIN ExpenseGroup EG On Ed.ExpenseGroupID=EG.ExpenseGroupID 
                                                    left join MastSalesRep msr on msr.SMID=Eg.SMID 
                                                    WHERE year(ED.billdate)='" + ddlYearExp.SelectedValue + "' and month(ED.billdate)='" + ddlMonthExp.SelectedValue + "' and Eg.IsSubmitted=1 and Eg.SMID IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN ( " + Settings.Instance.SMID + " ) AND level >= (SELECT DISTINCT level FROM   mastsalesrepgrp WHERE  maingrp IN ( " + Settings.Instance.SMID + " )) " + QryChk + ") GROUP  BY ED.billdate,SMName order by BillDate DESC";
        ...
        protected void Expenses_Click(object sender, EventArgs e)
        {
            ...
            BindSalePersonDDl();
            //            string expenseqry = @"SELECT ED.billamount,ED.billdate AS BillDate,ED.billnumber,ED.claimamount,Ed.ApprovedAmount,
            //                MET.NAME,msr.SMName, CASE ED.IsApprRej
            //                    WHEN 1 THEN 'Approved' when 2 then 'Reject' ELSE 'Pending' END AS Status
            //                FROM   expensedetails ED INNER JOIN mastexpensetype MET ON ED.expensetypeid = MET.id Inner Join ExpenseGroup EG On Ed.ExpenseGroupID=EG.ExpenseGroupID left join MastSalesRep msr on msr.SMID=Eg.SMID WHERE  Eg.IsSubmitted=1  and Eg.SMID=" + Settings.Instance.SMID + " GROUP  BY met.NAME,Ed.ApprovedAmount,ED.billamount,ED.billdate,ED.billnumber, ED.IsApprRej,ED.claimamount,msr.SMName order by BillDate desc";

            string expenseqry = @"SELECT sum(ED.billamount) AS billamount,ED.billdate AS BillDate,max(MET.Name) AS ExpenseName,sum(ED.claimamount) AS claimamount,
                                Sum(Ed.ApprovedAmount) AS ApprovedAmount,msr.SMName FROM expensedetails ED INNER JOIN mastexpensetype MET 
                                        ON ED.expensetypeid = MET.id Inner JOIN ExpenseGroup EG On Ed.ExpenseGroupID=EG.ExpenseGroupID 
                                        left join MastSalesRep msr on msr.SMID=Eg.SMID 
                                        WHERE year(ED.billdate)='" + Settings.GetUTCTime().Year + "' and month(ED.billdate)='" + Settings.GetUTCTime().Month + "' and Eg.IsSubmitted=1 and Eg.SMID IN (SELECT smid FROM   mastsalesrepgrp WHERE  maingrp IN ( " + Settings.Instance.SMID + " ) AND level >= (SELECT DISTINCT level FROM   mastsalesrepgrp WHERE  maingrp IN ( " + Settings.Instance.SMID + " ))) GROUP  BY ED.billdate,SMName order by BillDate DESC";
        ...
        protected void LeaveReq_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //                                  left join MastSalesRep msr on msr.SMId=r.SMId where r.SMId=" + Convert.ToInt32(smLogID) + " AND Convert(VARCHAR(10),r.FromDate,101)>='" + Settings.dateformat1(DateTime.Now.ToString()) + "' order by r.VDate desc";
        ...
        protected void PendingDSRApp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Dt2 = new DataTable();
                //                string str = @"select *,(select count(*) from TransVisit where TransVisit.SMId=a.SMId and TransVisit.AppStatus='Pending') as Pending from 
                //                   (select distinct TransVisit.SMId,MastSalesRep.SMName
                //                  from TransVisit 
                //                  left join MastSalesRep on TransVisit.SMId=MastSalesRep.SMId where [TransVisit].SMId in (select SMId from [MastSalesRep] where UnderId=" + smLogID + "))a";
                //Ankita - 09/may/2016- (For Optimization)
...
//                  from TransVisit left join MastSalesRep on TransVisit.SMId=MastSalesRep.SMId where [TransVisit].SMId in (select SMId from [MastSalesRep] where UnderId=" + Settings.Instance.SMID + ") and (TransVisit.AppStatus='Pending' or TransVisit.AppStatus is null))a)t where t.Pending<>0 ";
                string str = @"select SMName,EmpName,Pending,SMId from (select *,(select count(*) from TransVisit where TransVisit.SMId=a.SMId and (TransVisit.AppStatus='Pending' or TransVisit.AppStatus is null AND Transvisit.Lock=1)) as Pending from 
                   (select distinct TransVisit.SMId,MastSalesRep.SMName,MastSalesRep.EmpName
                  from TransVisit left join MastSalesRep on TransVisit.SMId=MastSalesRep.SMId where [TransVisit].SMId in (select SMId from [MastSalesRep] where UnderId=" + Settings.Instance.SMID + ") and (TransVisit.AppStatus='Pending' or TransVisit.AppStatus is null))a)t where t.Pending<>0 ";
...
//                " union ALL select mp.PartyName,ms.SMName,MAX(mig.ItemName) AS MaterialGroup,mic.Name AS ItemClass,0 as [Amount],0 as preAmt,convert(numeric(18, 2), sum(Amount/6)) as MonthAvg from TransDistInv1 tdi left join MastItem mi on tdi.ItemId=mi.ItemId left JOIN mastitem mig on mi.Underid=mig.ItemId left join MastItemClass mic on mic.Id=mi.ClassId left join MastItemSegment mis on mis.Id=mi.SegmentId left join MastParty MP on mp.PartyId=tdi.DistId  and mp.PartyDist=1 left join MastArea on MastArea.AreaId=MP.CityId left join transdistinv tdin on tdin.distinvdocid=tdi.distinvdocid LEFT JOIN mastsalesrep ms ON ms.SMId = tdin.SMID WHERE  mig.ItemType='materialGroup' and tdin.smid in (select smid from mastsalesrepgrp where maingrp = " + Settings.Instance.SMID + ") AND tdi.VDate between  '" + Settings.dateformat(dt_1.ToString()) + " 00:00'  and '" + Settings.dateformat(predt1.ToString()) + " 23:59' GROUP BY PartyName,mig.ItemName,mic.Name,mis.Name,ms.SMName )a group BY a.PartyName,a.materialGroup,a.itemClass,a.SMName";
        ...
        protected void LeaveApprv_Click(object sender, EventArgs e)
        {
        ...
        protected void tourFilterButton_Click(object sender, EventArgs e)
        {
            try
            {
                string tourmonthValue = tourMonthDDL.SelectedValue;
                string touryearValue = tourYearDDL.SelectedValue;

                DateTime date3; DateTime date4;
                if (tourmonthValue == "1")
                {
                    date3 = Convert.ToDateTime("1/" + (12) + "/" + (Settings.GetUTCTime().Year - 1));
                    date4 = Convert.ToDateTime("31/" + (12) + "/" + (Settings.GetUTCTime().Year - 1));
                ...
                {
                    date3 = Convert.ToDateTime("1/" + (Convert.ToInt32(tourmonthValue) - 1) + "/" + Convert.ToInt32(touryearValue));
                    date4 = Convert.ToDateTime(DateTime.DaysInMonth(Convert.ToInt32(touryearValue), Convert.ToInt32(tourmonthValue) - 1) + "/" + (Convert.ToInt32(tourmonthValue) - 1) + "/" + Convert.ToInt32(touryearValue));
                }

                string tourQuery = @" select TransTourPlan.VDate,TransTourPlan.MCityName AS Cityname,SMName,isnull((select sum(Qty* rate) from TransDistInv1  
                left join MastParty on MastParty.PartyId=TransDistInv1.DistId left join MastArea on MastArea.AreaId=MastParty.CityId  
                WHERE TransDistInv1.VDate BETWEEN '" + Settings.dateformat(date3.ToString()) + "' and '" + Settings.dateformat(date4.ToString()) + "' AND MastArea.AreaId=TransTourPlan.CityId),0) [Sale] from TransTourPlan left join MastArea ma1 on ma1.AreaId=TransTourPlan.CityId left join MastSalesRep msr on msr.SMId=TransTourPlan.SMId where TransTourPlan.SMId IN (SELECT smid FROM   mastsalesrepgrp WHERE  maingrp IN ( " + Settings.Instance.SMID + " ) AND level >= (SELECT DISTINCT level FROM   mastsalesrepgrp WHERE  maingrp IN ( " + Settings.Instance.SMID + " ))) and year(TransTourPlan.VDate)='" + touryearValue + "' and month(TransTourPlan.VDate)='" + tourmonthValue + "' and TransTourPlan.AppStatus='Approve' group by transtourplan.CityId,transtourplan.VDate,TransTourPlan.AccDistributor,TransTourPlan.SMId, ma1.AreaName,SMName,TransTourPlan.MCityName Order by TransTourPlan.VDate desc";
        ...
        public string Getdistributorid(int smid)
        {
            ...
            string distqry = @"select PartyId,PartyName from MastParty where CityId in (" + citystr + ") and smid in ((select maingrp from mastsalesrepgrp where smid in (" + Settings.Instance.SMID + ") union SELECT smid FROM mastsalesrepgrp WHERE  maingrp in (" + smid + ")))  and PartyDist=1 and Active=1 order by PartyName";
