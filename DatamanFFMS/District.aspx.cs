...
namespace AstralFFMS
{
    public partial class District : System.Web.UI.Page
    {
        ...
        private void BindParent(int StateId)
        {
            string strQ = "";
            if (StateId > 0)
                strQ = "select AreaID,AreaName from mastarea where AreaType='State' and (Active='1' or Areaid in (" + StateId + "))  order by AreaName";
            else
                strQ = "select AreaID,AreaName from mastarea where AreaType='State' and Active='1' order by AreaName";
        ...
        private void fillRepeter()
        {
            ...
            //string str = @"select ma.*,ma1.DisplayName as DisplayName1,case ma.Active when 1 then 'Yes' else 'No' end as Active1,ma1.AreaName as Parent from MastArea ma inner join MastArea ma1 on ma.UnderId=ma1.AreaID  where ma.AreaType='DISTRICT'";
            string str = @"select ma.AreaId,ma.AreaName,ma.SyncId,ma1.DisplayName as DisplayName1,case ma.Active when 1 then 'Yes' else 'No' end as Active1,ma1.AreaName as Parent from MastArea ma inner join MastArea ma1 on ma.UnderId=ma1.AreaID  where ma.AreaType='DISTRICT'";
        ...
        private void FillLocControls(int AreaId)
        {
            ...
            {//Ankita - 20/may/2016- (For Optimization)
                ...
                if (LocValueDt.Rows.Count > 0)
                {
                    ...
                    HiddenDisUnderID.Value = LocValueDt.Rows[0]["UnderId"].ToString();
        ...
        {
            string active = "0", StateID = Request.Form[HiddenDisUnderID.UniqueID];
            if (chkIsActive.Checked)
                active = "1";

            int retval = LB.InsertLocation(Convert.ToInt32(StateID), Location.Value, SyncId.Value, active, "", "DISTRICT", 0, 0, "", "", 0, 0, 0);
            if (retval == -1)
            {
                System.Web.UI.ScriptManager.RegisterStartupScript(this, GetType(), "errormessage", "errormessage('Duplicate District Exists');", true);
                Location.Focus();
            }
            else if (retval == -2)
            {
        ...
        private void UpdateLoc()
        {
            ...
            int retval = LB.UpdateLocation(Convert.ToInt32(ViewState["AreaId"]), Convert.ToInt32(StateID), Location.Value, SyncId.Value, active, "", "DISTRICT", 0, 0, "", "", 0, 0, 0);
        ...
        private void ClearControls()
        {
            Location.Value = "";
            SyncId.Value = "";
            HiddenDisUnderID.Value = string.Empty;
            chkIsActive.Checked = true;
