...
{
    public partial class MeetTargetFromLevel3 : System.Web.UI.Page
    {
        ...
        private void fillUnderLevel3()
        {
            ...
            if (d.Rows.Count > 0)
            {
                try
                {
                    DataView dv = new DataView(chkadmin_Active(d));
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (lnkState != null)
                {
                   ...
                   string smIDStr = "";
                    ...
                    string s = @"select * from MeetTragetFromHO Where  PartyTypeId=" + hid.Value + " and AssignedByID in (" + smIDStr1 + ") and MeetYear='" + txtcurrentyear.SelectedItem.Text + "'";
                    DataTable dtr2 = new DataTable();
                    try
                    {
                        dtr2 = DbConnectionDAL.GetDataTable(CommandType.Text, s);
                ...
                if (lnkArea != null)
                {
                    ...
                    //for (int i = 0; i < gauravDT1.Rows.Count; i++)
                    //{
                    string s = @"select count(*) from TransMeetPlanEntry where MeetTypeId=" + hid.Value + " and MeetDate>='" + Settings.GetStartFinancialYearfrmDate(txtcurrentyear.SelectedItem.Text) + "' and MeetDate<='" + Settings.GetStartFinancialYearToDate(txtcurrentyear.SelectedItem.Text) + "' and SMID in(" + sm1 + ") and AppStatus = 'Approved'";
                    try
                    {
            ...
            else if (e.CommandName == "State")
            {
                ...
                if (checkRole() == "RegionHead")
                {
                    lblpartyName1.Text = PartyDT.Rows[0][0].ToString();
                     ...
                     dv = new DataView(chkadmin_Active(d));
                 //    DataView dv = new DataView(chkadmin_Active());
        ...
        {
            Response.Redirect("~/MeetTargetFromLevel3.aspx");
        }
        ...
        //private DataTable chkadmin_Active(DataTable d)
        //{
        ...
        //    {
        //        string srty = "select * from MeetTragetFromHO where SMID=" + Settings.DMInt32(dv.Table.Rows[i]["smid"].ToString()) + " and PartyTypeId=" + lblPartTypeID.Text + " and MeetYear='" + txtcurrentyear.SelectedItem.Text + "'";

        //        dt12 = DbConnectionDAL.GetDataTable(CommandType.Text, srty);
        //        string s = d.Rows[i]["Active"].ToString();
        //        string isadminchk = DbConnectionDAL.GetScalarValue(CommandType.Text, "select count(*) from mastrole where roletype='Admin' and roleid=" + Settings.DMInt32(dv.Table.Rows[i]["roleid"].ToString()) + "").ToString();
        //        if ((isadminchk != "0") || (dv.Table.Rows[i]["Active"].ToString() == "False"))
        //        {
        //            if (dt12.Rows.Count == 0)
        //            {
        //                dv.Delete(i);
        ...
        {
            DataTable dt12 = new DataTable();
            d.Columns.Add("valueofI");
            int count = d.Rows.Count;
            for (int i = 0; i < count; i++)
            {
                d.Rows[i]["valueofI"] = i.ToString();
                string srty = "select * from SalesTragetFromHO where SMID=" + Settings.DMInt32(d.Rows[i]["smid"].ToString()) + " and PartyTypeId=" + lblPartTypeID.Text + " and SalesYear='" + txtcurrentyear.SelectedItem.Text + "'";
                dt12 = DbConnectionDAL.GetDataTable(CommandType.Text, srty);
                string s = d.Rows[i]["Active"].ToString();
                string isadminchk = DbConnectionDAL.GetScalarValue(CommandType.Text, "select count(*) from mastrole where roletype='Admin' and roleid=" + Settings.DMInt32(d.Rows[i]["roleid"].ToString()) + "").ToString();
                if ((isadminchk != "0") || (d.Rows[i]["Active"].ToString() == "False"))
                {
                    if (dt12.Rows.Count == 0)
                    {
                        d.Rows[i]["valueofI"] = "Delete";
                        d.AcceptChanges();
                    }
            ...
            {
                if (dr["valueofI"].ToString() == "Delete")
                    dr.Delete();
            }
            d.AcceptChanges();
            DataView dv = new DataView(d);
