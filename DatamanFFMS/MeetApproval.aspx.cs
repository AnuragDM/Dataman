...
{
    public partial class MeetApproval : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string sr = "select Count(*) from [MastMeetLogin] where userId in(" + Convert.ToInt32(Settings.Instance.UserID) + ") And IsAdmin2Type=0";
        ...
        private void fillGrid()
         {
             ...
             string MeetPlanID = Request.QueryString["MeetPlanID"];
                //string  sr = "select Count(*) from [MastMeetLogin] where userId in(" + Convert.ToInt32(Settings.Instance.UserID) + ")";
...
//            where smid in (select distinct smid from MastSalesRepGrp where smid in (select smid from MastSalesRepGrp where MainGrp in (" + Settings.Instance.SMID + ")) and  level> (select distinct level from MastSalesRepGrp where MainGrp in (" + Settings.Instance.SMID + " ))) and MastSalesRep.Active=1 order by MastSalesRep.SMName";
          ...
          if (MeetPlanID !=null)
          {
              str = @"select m.*,A.AreaName as CityName,A.[SyncId] as CityCode,MST.SMName, mp.PartyName + '-' + [city].areaName + '-' +[state].areaName  as Distributor,ms.Name as Scheme,dbo.getPartName(m.MeetPlanId) as PartyName,mb.AreaName as BeatName,mmtg.Name as typeOfGiftEnduser,dbo.getproduct(m.MeetPlanId) AS meetproduct from TransMeetPlanEntry m 
          ...
          {
              str = @"select m.*,A.AreaName as CityName,A.[SyncId] as CityCode,MST.SMName, mp.PartyName + '-' + [city].areaName + '-' +[state].areaName  as Distributor,ms.Name as Scheme,dbo.getPartName(m.MeetPlanId) as PartyName,mb.AreaName as BeatName,mmtg.Name as typeOfGiftEnduser,dbo.getproduct(m.MeetPlanId) AS meetproduct from TransMeetPlanEntry m 
                            left join MastArea A on m.MeetLoaction=A.AreaId Left Join MastSalesRep MST on MST.SMId = m.SMId Left Join MastParty mp on m.DistId=mp.PartyId  Left Join MastScheme ms on m.SchId=ms.Id left join MastParty MPP on m.RetailerPartyID=MP.PartyId left join MastArea mb on mb.AreaId=M.[AreaId] left join MastMeetTypeOfGift mmtg on m.TypeOfGiftId=mmtg.Id left join mastarea as [city] on [city].areaid=mp.cityid and [city].areatype='CITY' left join mastarea as [district] on [city].underid=[district].areaid left join mastarea as [state] on [district].underid=[state].areaid where m.SMId in (" + sm1 + ") and AppBy is NULL  order by m.MeetDate desc";
          }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            //{
                try
                {
                    ...
                    if (a)
                    {
                        string s = " Update TransMeetPlanEntry set AppStatus='" + ddlApp.SelectedItem.Text + "' ,AppRemark='" + txtRemarks.Text + "',AppBy=" + Settings.Instance.SMID + ", Appdate='" + Settings.dateformat1(DateTime.Now.ToShortDateString()) + "' , AppAmount=" + Settings.DMDecimal(txtApprovedBudget.Text) + "  where MeetPlanId=" + Settings.DMInt32(txtmeetId.Text);
                        ...
                        System.Web.UI.ScriptManager.RegisterStartupScript(this, GetType(), "Successmessage", "Successmessage('Meet Approved Successfully');", true);
                        btnUpdate.Enabled = false;
                    }
                    else
                    {
        ...
        private void InsertNotification()
        {
            try
            {
                ...
                string msgUrl = "MeetApproval.aspx?SMId=" + senSMid + "&MeetPlanId=" + txtmeetId.Text;
                ...
                if (ddlApp.Text == "Approved") ME.InsertTransNotification("MEETAPPROVED", Convert.ToInt32(salesRepqryForSManNewdt.Rows[0]["UserId"]), newDate12, msgUrl, "Meet Approved By - " + salesRepqueryNewdt.Rows[0]["SMName"]+" "+" "+"MeetName-"+MeetName+" ", 0, Convert.ToInt32(Settings.Instance.UserID), Convert.ToInt32(Settings.Instance.SMID), Convert.ToInt32(USMID.Rows[0]["SMId"]));
                else ME.InsertTransNotification("MEETREJECTED", Convert.ToInt32(salesRepqryForSManNewdt.Rows[0]["UserId"]), newDate12, msgUrl, "Meet  Rejected By - " + salesRepqueryNewdt.Rows[0]["SMName"] + " " + " " + "MeetName-" + MeetName + " ", 0, Convert.ToInt32(Settings.Instance.UserID), Convert.ToInt32(Settings.Instance.SMID), Convert.ToInt32(USMID.Rows[0]["SMId"]));
                ...
                if (ddlApp.Text == "Approved") ME.InsertTransNotificationL3("MEETAPPROVED", UserIDL3, newDate12, msgUrl1, "Meet Approved By - " + salesRepqueryNewdt.Rows[0]["SMName"] + " " + " " + "MeetName-" + MeetName + " ", 0, Convert.ToInt32(Settings.Instance.UserID), Convert.ToInt32(Settings.Instance.SMID), senSMid, Convert.ToInt32(salesRepID));
                else ME.InsertTransNotificationL3("MEETREJECTED", UserIDL3, newDate12, msgUrl1, "Meet Rejected By - " + salesRepqueryNewdt.Rows[0]["SMName"] + " " + " " + "MeetName-" + MeetName + " ", 0, Convert.ToInt32(Settings.Instance.UserID), Convert.ToInt32(Settings.Instance.SMID), senSMid, Convert.ToInt32(salesRepID));

                //Code for insert MastMeetLogin for all meetAdmin and Admin2Type
                string slqMeetAdmin = @"select UserId,SMID from MastMeetLogin Where IsAdmin2Type=1";
                DataTable MeetAdmin = DbConnectionDAL.GetDataTable(CommandType.Text, slqMeetAdmin);
                foreach (DataRow dr in MeetAdmin.Rows)
                {
                    if (dr["UserId"] != Settings.Instance.UserID) ME.InsertTransNotificationL3("MEETAPPROVED", Convert.ToInt32(dr["UserId"]), newDate12, msgUrl1, "Meet Approved By - " + salesRepqueryNewdt.Rows[0]["SMName"] + " " + " " + "MeetName-" + MeetName + " ", 0, Convert.ToInt32(Settings.Instance.UserID), Convert.ToInt32(Settings.Instance.SMID), Convert.ToInt32(dr["SMId"]), Convert.ToInt32(salesRepID));
        ...
        private void checkMeetAdmin()
        {
            try
            {
                ...
                sr = "select Count(*) from [MastMeetLogin] where userId in(" + Convert.ToInt32(Settings.Instance.UserID) + ") And IsAdmin2Type=0";
                int cnt = Convert.ToInt32(DbConnectionDAL.GetScalarValue(CommandType.Text, sr));
