...
{
    public partial class VarianceReportL3 : System.Web.UI.Page
    {
        ...
        {
            Response.Redirect("~/VarianceReportL3.aspx");
        }
                            ...
                            for (int cc = 0; cc < dvdt_NoOfdays.ToTable().Rows.Count; cc++)
                            {
                                if (dvdt_NoOfdays.ToTable().Rows[cc]["SMId"].ToString() == dr["SMId"].ToString())
                                {
                                    ...
                                    for (int c1 = 0; c1 <= dayloop; c1++)
                                    {
                                        ...
                                        {
                                            //Leave_str = "select cp.SMName,AppStatus,CONVERT (varchar,'" + Settings.dateformat(dateTime1.ToString()) + "' ,106) as [vdate],lr.LeaveString from TransLeaveRequest lr left join MastSalesRep cp on cp.SMId=lr.SMId where cp.SMId in (" + dt_NoOfdays.Rows[cc]["SMId"].ToString() + ") and lr.SMId in (" + dt_NoOfdays.Rows[cc]["SMId"].ToString() + ")  and lr.FromDate Between '" + Settings.dateformat(dt_NoOfdays.Rows[cc]["FromDate"].ToString()) + " 00:00' and '" + Settings.dateformat(dateTime1.ToString()) + " 23:59' and lr.appstatus in ('Approve','Pending')";
                                            ...
                                            foreach (DataRow dsr in dvdsrData.ToTable().Rows)
                                            {
                                                if (dsr["SMId"].ToString() == dr["SMId"].ToString())
                                                {
                                                    ...
                                                    {
                                                        if (dsr["Status"].ToString() == "Approve"){ls_val += "E/A";}
                                                        if (dsr["Status"].ToString() == "Reject"){ls_val += "E/R";}
                                                        if (dsr["Status"].ToString() == ""){ls_val += "E";}

                                                    }
                                                    //mDataRow["d" + Convert.ToDateTime(dsr["vdate"].ToString()).Day.ToString().Trim()] = dsr_val;
                                                    //dsr_val = "";
                                                }
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=VarianceReportL3.csv");
