...
{
    public partial class EditMeetPlan : System.Web.UI.Page
    {
...
(select linkcode from mastlink where primtable='SALESPERSON' and LinkTable='AREA' and PrimCode=" + ddlunderUser.SelectedValue + ") and  areatype='beat' and Active=1 order by AreaName";
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if ( k == 1)
                {
                    System.Web.UI.ScriptManager.RegisterStartupScript(this, GetType(), "errormessage", "errormessage('Your expenses has been approved');", true);
                }
                else
                {
        ...
        protected int MeetExpensecheck()
        {
            string strmeet = " SELECT count( * ) FROM TransMeetExpense WHERE FinalApprovedAmount IS NULL and  meetplanid= '" + MeetPlanID + "'";
            string meetexp = DAL.DbConnectionDAL.GetScalarValue(CommandType.Text, strmeet).ToString();
            int meetexpcheck = Convert.ToInt32(meetexp);
            if (meetexpcheck == 0 )
            {
                return 0;
            }
        ...
        {
            Response.Redirect("~/MeetProducts.aspx?MeetPlanID=" + MeetPlanID + "&Pagename=EditMeetplan");
        }
    }
}