...
namespace AstralFFMS
{
    public partial class ProjectMaster : System.Web.UI.Page
    {
        ...
        {
            string str = @"SELECT Id,Name,SyncId,CASE WHEN Active = 1 
                            THEN 'Yes' ELSE 'No'  END as active from MastProject";
        ...
        private void FillControls(int Id)
        {
            try
            { //Ankita - 20/may/2016- (For Optimization)
                //string Query = @"select * from MastProject where Id=" + Id;
                string Query = @"select Name,SyncId,Active from MastProject where Id=" + Id;
        ...
        private void Insert()
        {
            string str = @"select Count(*) from MastProject where Name='" + Name.Text + "'";
            ...
            if (val == 0)
            {
                ...
                {
                    string str1 = @"select Count(*) from MastProject where SyncId='" + SyncId.Value + "'";
                ...
                if (val1 == 0)
                {
                    ...
                    if (retsave != 0)
                    {
                        if (SyncId.Value == "")
                        {
                            string syncid = "update MastProject set SyncId='" + retsave + "' where Id=" + retsave + "";
        ...
        private void Update()
        {
            string strupd = @"select Count(*) from MastProject where Name='" + Name.Text + "' and Id<>" + Convert.ToInt32(ViewState["Id"]) + "";
            ...
            if (valupd == 0)
            {
                ...
                if (SyncId.Value != "" || SyncId.Value != string.Empty)
                {
                    string strupd1 = @"select Count(*) from MastProject where SyncId='" + SyncId.Value + "' and Id<>" + Convert.ToInt32(ViewState["Id"]) + "";
                ...
                if (valupd1 == 0)
                {
                    ...
                    if (retsave == 1)
                    {
                        if (SyncId.Value == "")
                        {
                            string syncid = "update MastProject set SyncId='" + ViewState["Id"] + "' where Id=" + Convert.ToInt32(ViewState["Id"]) + "";
                            DbConnectionDAL.ExecuteNonQuery(CommandType.Text, syncid);
                        }
