<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="RptRetaileritemWiseSale.aspx.cs" MasterPageFile="~/FFMS.Master" Inherits="AstralFFMS.RptRetaileritemWiseSale" %>
   ...
   <%-- <script src="<%=ResolveUrl("~")%>plugins/select2/select2.js"></script>
    ...
    <script type="text/javascript">
     ...
     <script type="text/javascript">
         ...
         function BindBeat() {
             $('#<%=ddlBeat.ClientID%>').empty();
              $.ajax({
                  type: "POST",
                  url: '<%= ResolveUrl("DataBindService.asmx/PopulateBeatByArea") %>',
                contentType: "application/json; charset=utf-8",
                data: '{AreaID: "' + $('#<%=ddlAreaPartylist.ClientID%>').val() + '" }',

                 dataType: "json",
                 success: function (data) {
                     jsdata1 = JSON.parse(data.d);                    
                     $('#<%=ddlBeat.ClientID%>').empty().append('<option selected="selected" value="0">-- Select --</option>');                    
                    $.each(jsdata1, function (key1, value1) {
                        $('#<%=ddlBeat.ClientID%>').append($("<option></option>").val(value1.BeatId).html(value1.BeatName));
         ...
         function BindParty() {
             $('#<%=ddlParty.ClientID%>').empty();
              $.ajax({
                  type: "POST",
                  url: '<%= ResolveUrl("DataBindService.asmx/PopulatePartyByBeat") %>',
                  contentType: "application/json; charset=utf-8",
                  data: '{BeatID: "' + $('#<%=ddlBeat.ClientID%>').val() + '" }',

                dataType: "json",
                success: function (data) {
                    jsdata1 = JSON.parse(data.d);                  
                    $('#<%=ddlParty.ClientID%>').empty().append('<option selected="selected" value="0">-- Select --</option>');                  
                     $.each(jsdata1, function (key1, value1) {
    ...
    <script type="text/javascript">
      ...
      function BindGridView() {
          $.ajax({
              type: "POST",
              url: "DistItemSaleReport.aspx/GetDistributorItemSale",
                        ...
                        <div id="InputWork">
                            ...
                            <div class="box box-primary">
                                <div class="box-header with-border">
                                    <h3 class="box-title">Distributor Item Sale</h3>
                                </div>
                                <!-- /.box-header -->
                                <!-- form start -->
                                <div class="box-body">
                                ...
                                <asp:DropDownList ID="ddlAreaPartylist" runat="server" onChange="BindBeat();" CssClass="form-control"></asp:DropDownList>
                               </div>
                                        </div>
                                ...
                                <asp:DropDownList ID="ddlBeat" runat="server" onChange="BindParty();" CssClass="form-control"></asp:DropDownList>
                               </div>
                                        </div> 
