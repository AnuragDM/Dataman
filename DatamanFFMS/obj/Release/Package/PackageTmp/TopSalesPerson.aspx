<%@ Page Language="C#" MasterPageFile="~/FFMS.Master" AutoEventWireup="true" CodeBehind="TopSalesPerson.aspx.cs" Inherits="AstralFFMS.TopSalesPerson" %>
                    ...
                    <div class="col-md-12">
                        <div id="InputWork">
                            ...
                            <div class="box box-primary">
                                <div class="box-header with-border">
                                    <%--<h3 class="box-title">Top Sales Person</h3>--%>
                        ...
                        <div id="rptmain" runat="server">
                            <div class="box-header table-responsive">
                                <asp:Repeater ID="TopRetailer" runat="server" OnItemCommand="TopRetailer_ItemCommand">          <%-- distitemsalerpt--%>
                                    <HeaderTemplate>
                                        <table id="itemsaleTable" class="table table-bordered table-striped">
                                            <thead>
                                                <tr>   
                                                    <th style="text-align: left; width: 10%">Sales Person</th>  
                                                    <th style="text-align: right; width: 10%">Total Primary Order Amount</th>                                              
                                                    <th style="text-align: right; width: 10%">Total Secondary Order Amount</th>
                                                    <th style="text-align: right; width: 10%">Total New Party</th>
                                                    <th style="text-align: right; width: 10%">Total Calls</th>
                                                    <th style="text-align: right; width: 10%">Prod Calls</th>
                                                </tr>
                                            </thead>
                                             <tfoot>
                                                <tr>
                                    ...
                                    <tr>
                                            ...
                                            <td><%# Eval("SPerson")%>
                                                <asp:Label ID="lblSPerson" runat="server" Visible="false" Text='<%# Eval("SPerson")%>'></asp:Label></td>   
                                         <td style="text-align: right">
                                            <asp:LinkButton CommandName="PrimaryOrder" ID="lnkPrimaryOrder" Text='<%#String.Format("{0:f2}",Eval("PrimaryOrder")) %>'  CausesValidation="False" runat="server"  ToolTip="Primary Order Details"
                                            Width="80px"  OnClientClick="window.document.forms[0].target='_blank'; setTimeout(function(){window.document.forms[0].target='';}, 500);" />                    
                                           <%-- <td style="text-align: right"><%# Eval("OrderAmount")%>--%>
                                                <asp:Label ID="lblPrimaryOrder" runat="server" Visible="false" Text='<%# String.Format("{0:f2}",Eval("PrimaryOrder"))%>'></asp:Label> </td>
                                        <td style="text-align: right">
                                            <asp:LinkButton CommandName="SecondaryOrder" ID="lnkEdit" Text='<%#Eval("OrderAmount") %>'  CausesValidation="False" runat="server"  ToolTip="Secondary Order Details"
                                            Width="80px"  OnClientClick="window.document.forms[0].target='_blank'; setTimeout(function(){window.document.forms[0].target='';}, 500);" />                    
                                           <%-- <td style="text-align: right"><%# Eval("OrderAmount")%>--%>
                                                <asp:Label ID="lblOrderAmount" runat="server" Visible="false" Text='<%# Eval("OrderAmount")%>'></asp:Label> </td>
                                            <%--<td style="text-align: right"><%# Eval("TotalNewParty")%>--%>
                                        <td style="text-align: right">
                                            <asp:LinkButton CommandName="Partyselect" ID="LinkButton1" Text='<%#Eval("TotalNewParty") %>'
                                            CausesValidation="False" runat="server"  ToolTip="Party Details"
                                            Width="80px" OnClientClick="window.document.forms[0].target='_blank'; setTimeout(function(){window.document.forms[0].target='';}, 500);" />  
                                                <asp:Label ID="lblTotalNewParty" runat="server" Visible="false" Text='<%# Eval("TotalNewParty")%>'></asp:Label></td> 
                                           <%-- <td style="text-align: right"><%# Eval("TotalCalls")%>--%>
                                        <td style="text-align: right">
                                             <asp:LinkButton CommandName="TotalCallsselect" ID="LinkButton2" Text='<%#Eval("TotalCalls") %>'
                                            CausesValidation="False" runat="server"  ToolTip="Call Details"
                                            Width="80px"  OnClientClick="window.document.forms[0].target='_blank'; setTimeout(function(){window.document.forms[0].target='';}, 500);" /> 
                                                <asp:Label ID="lblTotalCalls" runat="server" Visible="false" Text='<%# Eval("TotalCalls")%>'></asp:Label> </td>  
                                            <%--<td style="text-align: right"><%# Eval("ProdCalls")%>--%>
                                        <td style="text-align: right">
                                             <asp:LinkButton CommandName="ProdCallsselect" ID="LinkButton3" Text='<%#Eval("ProdCalls") %>'
                                            CausesValidation="False" runat="server"  ToolTip="Productive Details"
                                            Width="80px"  OnClientClick="window.document.forms[0].target='_blank'; setTimeout(function(){window.document.forms[0].target='';}, 500);" /> 
                                                <asp:Label ID="lblProdCalls" runat="server" Visible="false" Text='<%# Eval("ProdCalls")%>'></asp:Label>                </td>                              
                                   </tr>                                           
                                   </ItemTemplate>
                                    <FooterTemplate>
                                     </tbody>       
         ...
         $(function () {
             $('#itemsaleTable').dataTable({
                 ...
                 "footerCallback": function (tfoot, data, start, end, display) {
                     ...
                     var costColumnIndex1 = $('th').filter(function (i) { return $(this).text() == 'Total New Party'; }).first().index();
                     ...
                     var costColumnIndex2 = $('th').filter(function (i) { return $(this).text() == 'Total Calls'; }).first().index();
                     ...
                     var costColumnIndex3 = $('th').filter(function (i) { return $(this).text() == 'Prod Calls'; }).first().index();
