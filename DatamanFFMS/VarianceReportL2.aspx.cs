...
{
    public partial class VarianceReportL2 : System.Web.UI.Page
    {
        ...
        private void BindTreeViewControl()
        {
            try
            {
                ...
                if (roleType == "Admin")
                {
                    ...
                    St = DbConnectionDAL.GetDataTable(CommandType.Text, "Select msr.smid,msr.Smname +' ('+ msr.Syncid + ' - ' + mr.RoleName + ')' as smname,msr.underid,msr.lvl from mastsalesrep msr LEFT JOIN mastrole mr ON mr.RoleId=msr.RoleId where msr.active=1 and msr.underid<>0 and mr.Roletype in ('CityHead','DistrictHead','StateHead','RegionHead') order by msr.smname");
                }
                else
                {
                    string query = "select msr.smid,msr.Smname +' ('+ msr.Syncid + ' - ' + mr.RoleName + ')' as smname,msr.underid,msr.lvl from mastsalesrep msr LEFT JOIN mastrole mr ON mr.RoleId=msr.RoleId where msr.smid in (select smid from MastSalesRepGrp where MainGrp  in (" + Settings.Instance.SMID + ")) and  msr.active=1 and msr.lvl >= (select distinct level from MastSalesRepGrp  where MainGrp in (" + Settings.Instance.SMID + " )) and mr.Roletype in ('CityHead','DistrictHead','StateHead','RegionHead') order by msr.smname";
        ...
        {
            Response.Redirect("~/VarianceReportL2.aspx");
        }
        ...
        public void BindGrid()
        {
            ...
            try
            {
                ...
                mSRepCode = @"select SMId,SMName,SyncId from MastSalesRep where SMId in (SELECT ms.smid from mastsalesrep ms LEFT JOIN MastRole mr ON ms.RoleId= mr.RoleId WHERE ms.SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + smIDStr1 + ")) AND mr.RoleType IN ('CityHead','DistrictHead')) and Active=1 order by SMId";
                DataTable ssdt = DbConnectionDAL.GetDataTable(CommandType.Text, mSRepCode);

                //       DataTable ssdt = DataAccessLayer.DAL.getFromDataTable("select Name,Emp_Name from Contactpersons where Id in " + mSRepCode + " order by Name");

                //----------------------------- Leave -----------------------------------
                #region
                string Leave_str = "", strleave = "", status_pr = string.Empty, ls_val = string.Empty, dsr_val = string.Empty, DsrDate = string.Empty;
                double diff = 0; int dayloop = 0; string dsrQuery = string.Empty;
                ...
                string No_Days = "select NoOfDays,SMId,FromDate ,ToDate as NoDays,AppStatus,LeaveString from TransLeaveRequest lr where lr.SMId in (SELECT ms.smid from mastsalesrep ms LEFT JOIN MastRole mr ON ms.RoleId= mr.RoleId WHERE ms.SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + smIDStr1 + ")) AND mr.RoleType IN ('CityHead','DistrictHead')) and (lr.FromDate between '" + Settings.dateformat(mDate1.ToString()) + " 00:00' and '" + Settings.dateformat(mDate2.ToString()) + " 23:59' OR lr.ToDate between '" + Settings.dateformat(mDate1.ToString()) + " 00:00' and '" + Settings.dateformat(mDate2.ToString()) + " 23:59') and lr.appstatus in ('Approve','Pending') order by smid,FromDate";
                ...
                foreach (DataRow dr in ssdt.Rows)
                {
                    ...
                    foreach (DataRow dsr1 in dvdsrData.ToTable().Rows)
                    {
                        ...
                        {
                            if (dsr1["Status"].ToString() == "Approve") { mDataRow["d" + DsrDate] = "E/A"; }
                        ...
                        for (int cc = 0; cc < dvdt_NoOfdays.ToTable().Rows.Count; cc++)
                        {
                            if (dvdt_NoOfdays.ToTable().Rows[cc]["SMId"].ToString() == dr["SMId"].ToString())
                            {
                                ...
                                for (int c1 = 0; c1 <= dayloop; c1++)
                                {
                                    ...
                                    {
                                        //Leave_str = "select cp.SMName,AppStatus,CONVERT (varchar,'" + Settings.dateformat(dateTime1.ToString()) + "' ,106) as [vdate],lr.LeaveString from TransLeaveRequest lr left join MastSalesRep cp on cp.SMId=lr.SMId where lr.FromDate Between '" + Settings.dateformat(dt_NoOfdays.Rows[cc]["FromDate"].ToString()) + " 00:00' and '" + Settings.dateformat(dateTime1.ToString()) + " 23:59' and lr.SMId in (SELECT ms.smid from mastsalesrep ms LEFT JOIN MastRole mr ON ms.RoleId= mr.RoleId WHERE ms.smid IN (SELECT smid FROM MastSalesRep WHERE SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + dt_NoOfdays.Rows[cc]["SMId"].ToString() + ") AND level >= (SELECT DISTINCT level FROM   mastsalesrepgrp WHERE  maingrp IN (" + dt_NoOfdays.Rows[cc]["SMId"].ToString() + "))) AND mr.roletype='CityHead' or RoleType='DistrictHead'))";
                                        //Leave_str = "select cp.SMName,AppStatus,CONVERT (varchar,'" + Settings.dateformat(dateTime1.ToString()) + "' ,106) as [vdate],lr.LeaveString from TransLeaveRequest lr left join MastSalesRep cp on cp.SMId=lr.SMId where lr.FromDate Between '" + Settings.dateformat(dt_NoOfdays.Rows[cc]["FromDate"].ToString()) + " 00:00' and '" + Settings.dateformat(dateTime1.ToString()) + " 23:59' and lr.SMId in (SELECT ms.smid from mastsalesrep ms LEFT JOIN MastRole mr ON ms.RoleId= mr.RoleId WHERE ms.SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + dt_NoOfdays.Rows[cc]["SMId"].ToString() + ")) AND mr.RoleType IN ('CityHead','DistrictHead')) select cp.SMName,AppStatus,CONVERT (varchar,'04/23/2016' ,106) as [vdate],lr.LeaveString from TransLeaveRequest lr left join MastSalesRep cp on cp.SMId=lr.SMId where lr.FromDate Between '04/23/2016 00:00' and '04/23/2016 23:59' and lr.SMId in (SELECT ms.smid from mastsalesrep ms LEFT JOIN MastRole mr ON ms.RoleId= mr.RoleId WHERE ms.SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (1423)) AND mr.RoleType IN ('AreaIncharge')) and lr.appstatus in ('Approve','Pending')";
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            if (gvData.Rows.Count != 0)
            {
                ...
                foreach (GridViewRow row in gvData.Rows)
                {
                    ...
                    {
                        //          dr[gvData.HeaderRow.Cells[j].Text] = row.Cells[j].Text;
            ...
            Response.AddHeader("content-disposition", "attachment;filename=VarianceReportL2.csv");
            Response.Write(sb.ToString());
            Response.End();

            //Response.Clear();
            //Response.Buffer = true;
            //Response.AddHeader("content-disposition", "attachment;filename=VarianceReportL2.xls");
            //Response.Charset = "";
        ...
        protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            try
            {
                string mDay = ""; string str = String.Empty; string filledVal = String.Empty; int ptChange = 0; int checkChange = 0; int changeFlag = 0; Boolean chkNext = false;
