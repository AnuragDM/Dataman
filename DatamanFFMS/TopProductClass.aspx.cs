...
{
    public partial class TopProductClass : System.Web.UI.Page
    {
        string roleType = "";
        int cnt = 0;
        string orderType = "";
        ...
        private void BindProductClass()
        {
            try
            {
                string prodClassQry = @"SELECT Id,Name FROM MastItemClass WHERE Active=1 ORDER BY Name";
        ...
        protected void btnGo_Click(object sender, EventArgs e)
        {
            ...
            if (smIDStr1 != "")
            {
                ...
                string query = "SELECT TOP " + txt_noofrecords.Text + " ClassId,ProductClass,convert(int,sum(Qty)) [Qty],convert(decimal, sum(Amount)) [Amount] FROM(SELECT pg.Id as ClassId,pg.Name [ProductClass],convert(int,(os.Qty)) [Qty],convert(decimal, (os.Qty * os.Rate)) [Amount] From  TransOrder1 os left JOIN MastItem i ON os.ItemId = i.ItemId ";
                query = query + "left JOIN mastitemclass pg ON pg.Id = i.ClassId " + Qrychk + " " + QueryMatGrp + " Union All SELECT pg.Id as ClassId,pg.Name [ProductClass],convert(int,(os.Qty)) [Qty],convert(decimal, (os.Qty * os.Rate)) [Amount] From Temp_TransOrder1 os left JOIN MastItem i ON os.ItemId = i.ItemId ";
                query = query + "left JOIN mastitemclass pg ON pg.Id = i.ClassId " + Qrychk + " " + QueryMatGrp + ") a  GROUP BY a.ClassId,a.ProductClass Order by sum(a.Amount) " + order + "";
        ...
        {
            Response.Redirect("~/TopProductClass.aspx");
        }
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=topproductClass.csv");
            string headertext = "Product Class".TrimStart('"').TrimEnd('"') + "," + "Qty".TrimStart('"').TrimEnd('"') + "," + "Amount".TrimStart('"').TrimEnd('"');
        ...
        protected void Topsaleproduct_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "select")
            {
                ...
                Response.Redirect("TopProductClassDetails.aspx?itemid=" + hdnitemid.Value + "&FromDate=" + txtfmDate.Text + "&ToDate=" + txttodate.Text + "&smidstr='" + ViewState["smIDStr1"].ToString() + "'");
            }
        }
    }
}