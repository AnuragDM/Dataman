...
{
    public partial class DistributerPurchOrderReport : System.Web.UI.Page
    {
        string roleType = "";
        int cnt = 0;
        ...
        public static string GetDistributorItemSale1(string PODOCID)
        {
            string qry = "";
          
            string query = @"SELECT t1.VDate,da.Syncid,da.PartyName AS Distributor,PODocid,max(mi1.ItemName) AS MaterialGROUP, mi.ItemName AS [Item],sum(qty) as Qty,isnull(rate,0) as rate, isnull(sum(rate*qty),0)as amount from TransPurchOrder1 t1 left outer join MastItem mi
             ...
             PODocid in ('" + PODOCID + "')  and da.partydist=1 group BY t1.VDate,da.syncid,da.PartyName,PODocid,mi.ItemName,rate order BY  t1.VDate desc";
        ...
        public static string GetDistributorItemSale(string Distid, string ProductGroup, string Product, string Fromdate, string Todate)
        {
            ...
            string query = @"SELECT t1.VDate,da.Syncid,da.PartyName AS Distributor,PODocid,max(mi1.ItemName) AS MaterialGROUP, mi.ItemName AS [Item],sum(qty) as Qty,isnull(rate,0) as rate, isnull(sum(rate*qty),0)as amount from TransPurchOrder1 t1 left outer join MastItem mi
             on mi.ItemId=t1.ItemId LEFT JOIN mastitem mi1 ON mi1.ItemId=mi.Underid left outer join MastParty da on da.PartyId=t1.DistId where 
             t1.DistId in (" + Distid + ") and " + Qrychk + "  " + qry + " and da.partydist=1 group BY t1.VDate,da.syncid,da.PartyName,PODocid,mi.ItemName,rate order BY  t1.VDate desc";
            DataTable dtItem = DbConnectionDAL.GetDataTable(CommandType.Text, query);
            return JsonConvert.SerializeObject(dtItem);
        ...
        {
            Response.Redirect("~/DistributerPurchOrderReport.aspx");
        }
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=distributerpurchorderreport.csv");
            string headertext = "Date".TrimStart('"').TrimEnd('"') + "," + "Distributor Sync Id".TrimStart('"').TrimEnd('"') + "," + "Distributor Name".TrimStart('"').TrimEnd('"') + "," + "Order No.".TrimStart('"').TrimEnd('"') + "," + "Product Group".TrimStart('"').TrimEnd('"') + "," + "Product".TrimStart('"').TrimEnd('"') + "," + "Qty".TrimStart('"').TrimEnd('"') + "," + "Rate".TrimStart('"').TrimEnd('"') + "," + "Amount".TrimStart('"').TrimEnd('"');
           ...
           //  t1.DistId in (" + distIdStr1 + ") and " + Qrychk + "  " + QueryMatGrp + " and da.partydist=1 group BY t1.VDate,da.syncid,da.PartyName, mi.ItemName order BY  t1.VDate desc";
                    string query = @"SELECT t1.VDate,da.Syncid,da.PartyName AS Distributor,PODocId,max(mi1.ItemName) AS MaterialGROUP, mi.ItemName AS [Item],sum(qty) as Qty,isnull(rate,0) as rate, isnull(sum(rate*qty),0)as amount from TransPurchOrder1 t1 left outer join MastItem mi
             on mi.ItemId=t1.ItemId LEFT JOIN mastitem mi1 ON mi1.ItemId=mi.Underid left outer join MastParty da on da.PartyId=t1.DistId where 
             t1.DistId in (" + distIdStr1 + ") and " + Qrychk + "  " + QueryMatGrp + " and da.partydist=1 group BY t1.VDate,da.syncid,da.PartyName,PODocId,mi.ItemName,rate order BY  t1.VDate desc";
