...
{
    public partial class TotalCallsDetails : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                lbltdate.Text = To;
                lbltotalcalls.Text = smid.ToString();
                fillRepeter();
                fillRepeter1();
            }
        }
        private void fillRepeter()
        {
            ...
            string query = @"SELECT a.smid,a.vdate,a.Visid,a.partyid,a.beatid,a.SMName,a.PartyName,a.Address1,a.Mobile FROM (SELECT om.SMID,om.VDate,om.VisId, om.PartyId,P.BeatId,ms.SMName,p.PartyName,p.Address1,p.Mobile FROM TransOrder om LEFT JOIN mastparty p ON p.PartyId=om.PartyId LEFT JOIN mastsalesrep ms ON om.SMId=ms.SMId UNION ALL SELECT om.SMID,om.VDate,om.VisId, om.PartyId,P.BeatId,ms.SMName,p.PartyName,p.Address1,p.Mobile FROM Temp_TransOrder om LEFT JOIN mastparty p ON p.PartyId=om.PartyId LEFT JOIN mastsalesrep ms ON om.SMId=ms.SMId UNION SELECT om.SMID,om.VDate,om.VisId, om.PartyId,P.BeatId,ms.SMName,p.PartyName,p.Address1,p.Mobile FROM TransDemo om LEFT JOIN mastparty p ON p.PartyId=om.PartyId LEFT JOIN mastsalesrep ms ON om.SMId=ms.SMId UNION ALL SELECT om.SMID,om.VDate,om.VisId, om.PartyId,P.BeatId,ms.SMName,p.PartyName,p.Address1,p.Mobile FROM Temp_TransDemo om LEFT JOIN mastparty p ON p.PartyId=om.PartyId LEFT JOIN mastsalesrep ms ON om.SMId=ms.SMId UNION ALL SELECT om.SMID,om.VDate,om.VisId,om.PartyId,P.BeatId,ms.SMName,p.PartyName,p.Address1,p.Mobile FROM TransFailedVisit om LEFT JOIN mastparty p ON p.PartyId=om.PartyId LEFT JOIN mastsalesrep ms ON om.SMId=ms.SMId WHERE p.PartyDist=0 UNION ALL SELECT om.SMID,om.VDate,om.VisId,om.PartyId,P.BeatId,ms.SMName,p.PartyName,p.Address1,p.Mobile FROM Temp_TransFailedVisit om LEFT JOIN mastparty p ON om.PartyId=p.PartyId LEFT JOIN mastsalesrep ms ON om.SMId=ms.SMId WHERE p.PartyDist=0)a LEFT JOIN mastsalesrep ms ON a.smid=ms.SMId WHERE a.VDate>='" + From + " 00:00' and VDate<='" + To + " 23:59' AND a.smid IN (SELECT smid FROM MastSalesRep WHERE SMId IN (SELECT smid FROM mastsalesrepgrp WHERE  maingrp IN (" + smid + ")) and Active=1)";

           
            DataTable dt = DbConnectionDAL.GetDataTable(CommandType.Text, query);
            Totalcalls.DataSource = dt;
            Totalcalls.DataBind();
        ...
        private void fillRepeter1()
        {
            ...
            lbltotalcalls.Text = dt.Rows[0][1].ToString();
        ...
        protected void btnExport(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=TotalCallsDetails.csv");
            ...
            foreach (RepeaterItem item in Totalcalls.Items)
            {
