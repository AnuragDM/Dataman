...
{
    public partial class TopProductSegment : System.Web.UI.Page
    {
        string roleType = "";
        int cnt = 0;
        string orderType = "";
        ...
        private void BindProductSegment()
        {
            try
            {
                string prodClassQry = @"SELECT Id,Name FROM MastItemSegment WHERE Active=1 ORDER BY Name";
                ...
                if (dtProdRep.Rows.Count > 0)
                {
                    ...
                    matGrpListBox.DataTextField = "Name";
                    matGrpListBox.DataValueField = "Id";
        ...
        protected void btnGo_Click(object sender, EventArgs e)
        {
            ...
            if (matGrpStrNew != "" && matProStrNew == "")
            { QueryMatGrp = " and pg.Id in (" + matGrpStrNew + ") "; }
            if (smIDStr1 != "")
            {
                ...
                //string query = "SELECT TOP " + txt_noofrecords.Text + " pg.ItemName [ProductGroup],i.itemid,i.ItemName [Product],convert(int,sum(os.Qty)) [Qty],convert(decimal, sum(os.Qty * os.Rate)) [Amount] FROM TransOrder1 os left JOIN MastItem i ON os.ItemId = i.ItemId ";
                //query = query + "left JOIN MastItem pg ON pg.ItemId = i.Underid " + Qrychk + " " + QueryMatGrp + " GROUP BY i.ItemId, i.ItemName, pg.ItemName Order by sum(os.Qty * os.Rate) " +order+"";

                string query = "SELECT TOP " + txt_noofrecords.Text + " ProductSegmentId,ProductSegment,convert(int,sum(Qty)) [Qty],convert(decimal, sum(Amount)) [Amount] FROM(SELECT pg.Id as ProductSegmentId,pg.Name [ProductSegment],convert(int,(os.Qty)) [Qty],convert(decimal, (os.Qty * os.Rate)) [Amount] From  TransOrder1 os left JOIN MastItem i ON os.ItemId = i.ItemId ";
                query = query + "left JOIN mastitemsegment pg ON pg.Id = i.SegmentId " + Qrychk + " " + QueryMatGrp + " Union All SELECT pg.Id as ProductSegmentId,pg.Name [ProductSegment],convert(int,(os.Qty)) [Qty],convert(decimal, (os.Qty * os.Rate)) [Amount] From Temp_TransOrder1 os left JOIN MastItem i ON os.ItemId = i.ItemId ";
                query = query + "left JOIN mastitemsegment pg ON pg.Id = i.SegmentId " + Qrychk + " " + QueryMatGrp + ") a  GROUP BY a.ProductSegmentId,a.ProductSegment Order by sum(a.Amount) " + order + "";
        ...
        {
            Response.Redirect("~/TopProductSegment.aspx");
        }
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=topproductSegment.csv");
            string headertext = "Product Segment".TrimStart('"').TrimEnd('"') + "," + "Qty".TrimStart('"').TrimEnd('"') + "," + "Amount".TrimStart('"').TrimEnd('"');
        ...
        protected void Topsaleproduct_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "select")
            {
                ...
                Response.Redirect("TopProductSegmentDetails.aspx?itemid=" + hdnitemid.Value + "&FromDate=" + txtfmDate.Text + "&ToDate=" + txttodate.Text + "&smidstr='" + ViewState["smIDStr1"].ToString() + "'");
            }
        }
    }
}