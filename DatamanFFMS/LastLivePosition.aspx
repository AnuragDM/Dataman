<%@ Page Title="" Language="C#" MasterPageFile="~/FFMS.Master" AutoEventWireup="true" CodeBehind="LastLivePosition.aspx.cs" Inherits="AstralFFMS.LastLivePosition" %>
    ...
    function initialize() {
        ...
        for (i = 0; i < markers.length; i++) {
            ...
            var marker = new google.maps.Marker({
                position: myLatlng,
                map: map,
                title: data.smname+'  at  '+data.description,
          ...
          '</span><div id="bodyContent">'+data.description+'<br>Visit Type : Regular</br>Time:'+data.time+'</br>Sequence : '+data.sno+'</div>'+'</div>';
                ...
                google.maps.event.addListener(marker, "click", function (e) {
                    ...
                    GetRouteByPerson(data.smid,data.date);
                    ShowModalPopup();
                });
            })(marker, data);
 ...
 <h3> <span class="pull-left" id="ContentPlaceHolder1_lblpagename">Live Map</span></h3>
              ...
              <div class="box-body table-responsive force-overflow">
                                    <asp:Repeater ID="leavereportrpt" runat="server"  >
                                            ...
                                            <table id="example1" class="table ">
                                                <thead>
                                                    <tr style="border-radius:4px">
                                                        <th style="background-color:#3c8dbc;color:white">SalesPerson</th>
                                        ...
                                        <ItemTemplate>                             
                                            <tr>
                                                <td>                                        
                                                 <a href="#" onclick="myClick(<%#Container.ItemIndex %>);"><%#Eval("smname") %></a>                                                 
                                                 <td><%#Eval("time") %>      
                                                     <%-- <td><%# System.Convert.ToDateTime(Eval("time")).ToString("dd/MMM/yyyy HH:MM:ss")%>      --%>                                    
                                                </td>                                              
                                            </tr>                                                
                                        </ItemTemplate>
                                        <FooterTemplate>
  ...
  <table style="width:100%;">
    <tr>
      ...
      <td>  <div id="dvMap" style="width: 100%; height: 500px;border-style: inset;">
  ...
  <%--<div class="dvInsideTL" id="divContent">
         ...
         <asp:Panel ID="pnlItem" runat="server" Style="display: none; background-color:White;" Width="945px" Height="600" ScrollBars="Vertical">
              ...
              <img id="imgclose" style="float:right;" src="img/delete.png" onclick="HideModalPopup();" />
                        ...
                        <div class="popupdiv">
                           ...
                           <td>
                          <div id="Div_Route" style="width: 1200px; height: 550px"></div>
    ...
    <script type="text/javascript">
        ...
        function GetRouteByPerson(smid,date) {
            var pageUrl = '<%=ResolveUrl("~/And_sync.asmx")%>';
            
            $.ajax({
                type: "POST",
                url: pageUrl + '/GetRouteByPerson',
                contentType: "application/json; charset=utf-8",
                data: "{'smid':'" + smid + "','date':'"+ date +"'}",
                dataType: "json",
    ...
    <script type="text/javascript">
        ...
        function initializeRoute(jsondata) {
            ...
            var objmid = JSON.parse(json)[Math.round(JSON.parse(json).length/2)];
            //alert(Math.round(JSON.parse(json).length/2));
            var directionsDisplay = new google.maps.DirectionsRenderer();
            var lat_lng = [];
            var latlngbounds = "";
            var mapOptions = {
                center: new google.maps.LatLng(objmid.Lat,objmid.Lng),
                zoom: 15,
            ...
            for (var i = 0; i < JSON.parse(json).length; i += 1) {
                ...
                (function (marker2, obj) {
                    var contentString = '<div id="content" style="font-size:11px;width:250px;height:200px;background-color: lightblue"  ><span style="color:blue;font-weight:bold">'+''+obj.Title+''+
    '</span><div id="bodyContent">'+obj.address1+'<br>Visit Type : Regular</br>Time:'+obj.Time+'</br>Sequence : '+obj.SNo+'</br><img src='+obj.imgurl+' alt="Image" height="100" width="100"></div>'+'</div>';
