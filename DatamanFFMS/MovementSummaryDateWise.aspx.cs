...
{
    public partial class MovementSummaryDateWise : System.Web.UI.Page
    {
        ...
        {
            string str = "select GroupMaster.GroupID as Code, GroupMaster.Description as Description from UserGrp inner Join GroupMaster on GroupMaster.GroupID = UserGrp.GroupId where UserGrp.UserID = '" + Settings.Instance.Track_userid + "' order by GroupMaster.Description";
        ...
        protected void btnexport_Click(object sender, EventArgs e)
        {
            try
            {
                GridViewExportUtil.Export("MovementSummaryDateWise", gdw);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                for (int p = 0; p <= loops; p++)
                {
                    ...
                    for (int k = 0; k < dttemp.Rows.Count; k++)
                    {
                        if (k == p)
                        {
                            Fqry += " case status when 'GO' then ' GPS Off' when 'LO' then ' Location Off' when 'MO' then ' Internet Problem' when 'M' then ' Datetime is set manually' when 'TN' then ' Tower not in reach' end as '" + dttemp.Rows[k]["Slot"] + "',";
                        }
                        else
                        {
                            Fqry += "' ' as '" + dttemp.Rows[k]["Slot"] + "',";
                ...
                for (int p = 0; p <= loops; p++)
                {
                    ...
                    Fqry = Fqry.Substring(0, Fqry.Length - 1);
                    Fqry += " from log_tracker RIGHT JOIN PersonMaster ON log_tracker.DeviceNo=PersonMaster.DeviceNo  " +
                        " where log_tracker.deviceno in (" + addDeviceNo + ") and   cast(CurrentDate as date) >=cast('" + txtfromdate.Text + "' as date) AND cast(CurrentDate as date) < = cast('" + txtfromdate.Text + " ' as date)" +
                        " and status='LV' ";
                    Fqry += " union ";
                }
