...
namespace AstralFFMS
{
    ...
    {
        BAL.ClassMaster.ClassMasterBAL dp = new BAL.ClassMaster.ClassMasterBAL();
        string parameter = "";
        ...
        private void fillRepeter()
        {
            string str = @"SELECT Id,Name,SyncCode,CASE WHEN Active = 1 
                            THEN 'Yes' ELSE 'No'  END as active from MastItemClass";
        ...
        private void FillControls(int Id)
        {
            try
            {
                string Query = @"select * from MastItemClass where Id=" + Id;
        ...
        private void Insert()
        {
            string str = @"select Count(*) from MastItemClass where Name='" + Name.Text + "'";
            ...
            if (val == 0)
            {
                ...
                {
                    string str1 = @"select Count(*) from MastItemClass where SyncCode='" + SyncId.Value + "'";
                ...
                if (val1 == 0)
                {
                    ...
                    if (retsave != 0)
                    {
                        if (SyncId.Value == "")
                        {
                            string syncid = "update MastItemClass set SyncCode='" + retsave + "' where id=" + retsave + "";
        ...
        private void Update()
        {
            string strupd = @"select Count(*) from MastItemClass where Name='" + Name.Text + "' and Id<>" + Convert.ToInt32(ViewState["Id"]) + "";
            ...
            if (valupd == 0)
            {
                ...
                if (SyncId.Value != "" || SyncId.Value != string.Empty)
                {
                    string strupd1 = @"select Count(*) from MastItemClass where SyncCode='" + SyncId.Value + "' and Id<>" + Convert.ToInt32(ViewState["Id"]) + "";
                ...
                if (valupd1 == 0)
                {
                    ...
                    if (retsave == 1)
                    {
                        if (SyncId.Value == "")
                        {
                            string syncid = "update MastItemClass set SyncCode='" + ViewState["Id"].ToString() + "' where id=" + Convert.ToInt32(ViewState["Id"]) + "";
                            DbConnectionDAL.ExecuteNonQuery(CommandType.Text, syncid);
                        }
        ...
        {
            ClearControls();
            Response.Redirect("~/ClassMaster.aspx");
        }
