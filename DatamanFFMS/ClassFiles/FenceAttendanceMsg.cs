...
/// <summary>
/// Summary description for FenceAttendanceMsg
/// </summary>
[WebService(Namespace = "http://tempuri.org/")]
[WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
// To allow this Web Service to be called from script, using ASP.NET AJAX, uncomment the following line. 
 [System.Web.Script.Services.ScriptService]
public class FenceAttendanceMsg : System.Web.Services.WebService {
    UploadData Upd = new UploadData();
    static string Flong = "", Flat = "";
    #region GetSavePersonForFenceAttendance Msg
    [WebMethod]
    public string GetAttendanceFence(string hdnlatlng)
    {
        string addr = "";
        try
        {
    ...
    public string GetPerson(string GroupId)
    {
        ...
        dtper = DbConnectionDAL.getFromDataTable(strq);
    ...
    [WebMethod]
    public string SaveFenceAttendance(string Fradius, string Faddr, string FPersonId, string FgroupId)
    {
        try
        {
            Fradius = Math.Round((Convert.ToDecimal(Fradius)), 3).ToString();
            Upd.InsertUpdateFenceAttendanceAlert(Flat, Flong, Fradius, Faddr, FPersonId, FgroupId);
