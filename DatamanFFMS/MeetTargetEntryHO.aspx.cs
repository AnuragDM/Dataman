...
{
    public partial class MeetTargetEntry : System.Web.UI.Page
    {
        ...
        private void fillUnderLevel3()
        {
            ...
            DataTable d = Settings.UnderUsers1(Settings.Instance.SMID); DataView dv = new DataView(chkadmin_Active(d));
            if (d.Rows.Count > 0)
            {
                try
                {
        ...
        private void fillbyL1()
        {
           // string st = "select count(*) from TransMeetPlanEntry where MeetTypeId=";
        }
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (lnkArea != null)
                {
                    ...
                    //for (int i = 0; i < gauravDT1.Rows.Count; i++)
                    //{
                    string s = @"select count(*) from TransMeetPlanEntry where  MeetDate>='" + Settings.GetStartFinancialYearfrmDate(txtcurrentyear.SelectedItem.Text) + "' and MeetDate<='" + Settings.GetStartFinancialYearToDate(txtcurrentyear.SelectedItem.Text) + "' and MeetTypeId=" + hid.Value + " and SMID in(" + sm1 + ")";
                    try
                    {
        ...
        {
            string st = @"select RoleName from MastSalesRep M left join MastRole R on M.RoleId=R.RoleId where SMId=" + Settings.DMInt32(Settings.Instance.SMID);
            string RName = Convert.ToString(DbConnectionDAL.GetScalarValue(CommandType.Text, st));
            return RName;
        }

        private void fillPlannedbyL1()
        {
            try
            {
        ...
        {
            Response.Redirect("~/MeetTargetEntryHO.aspx");
        }
        ...
        //    {
        //        //tring srty = "select * from SalesTragetFromHO where SMID=" + Settings.DMInt32(PartyId.Value) + " and PartyTypeId=" + lblPartTypeID.Text + " and SalesYear='" + txtcurrentyear.SelectedItem.Text + "'";
        //        string srty = "select * from MeetTragetFromHO where SMID=" + Settings.DMInt32(d.Rows[i]["smid"].ToString()) + " and PartyTypeId=" + lblPartTypeID.Text + " and MeetYear='" + txtcurrentyear.SelectedItem.Text + "'";
