...
{
    public partial class AttendanceNew : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                foreach (DataRow dr in ssdt.Rows)
                {
                    ...
                    foreach (DataRow dsr1 in dvdsrData.ToTable().Rows)
                    {
                        ...
                        {
                            if (dsr1["Status"].ToString() == "Approve") { mDataRow["d" + DsrDate] = "P"; }
                            if (dsr1["Status"].ToString() == "Reject") { mDataRow["d" + DsrDate] = "E/R"; }
                            if (dsr1["Status"].ToString() == "") { 

                                mDataRow["d" + DsrDate] = "E";
                                string intime = DbConnectionDAL.GetStringScalarVal("select top 1 convert(char(5), mobile_created_date, 108) [Intime] from view_getfirsttranstime  where smid=" + dsr1["SMId"].ToString() + " and vdate='" + Convert.ToDateTime(dsr1["vdate"].ToString()).ToString("yyyy-MM-dd") + "' order by  mobile_created_date");
                                string Outtime = DbConnectionDAL.GetStringScalarVal("select top 1 convert(char(5), mobile_created_date, 108) [Intime] from view_getfirsttranstime  where smid=" + dsr1["SMId"].ToString() + " and vdate='" + Convert.ToDateTime(dsr1["vdate"].ToString()).ToString("yyyy-MM-dd") + "' order by  mobile_created_date desc");
                                string sql = "select  DATEDIFF(HOUR,(select top 1 mobile_created_date from view_getfirsttranstime  where smid=" + dsr1["SMId"].ToString() + " and vdate='" + Convert.ToDateTime(dsr1["vdate"].ToString()).ToString("yyyy-MM-dd") + "' order by  mobile_created_date),(select top 1  mobile_created_date from view_getfirsttranstime  where smid=" + dsr1["SMId"].ToString() + " and vdate='" + Convert.ToDateTime(dsr1["vdate"].ToString()).ToString("yyyy-MM-dd") + "' order by mobile_created_date desc)) ";
                                string Duration = DbConnectionDAL.GetStringScalarVal(sql);

                                mDataRow["d" + DsrDate]= intime+"-"+Outtime +" , "+Duration;
                            }
                        }
                    }
