...
{
    public partial class RolePermissionAndroid : System.Web.UI.Page
    {
        Data d = new Data();
        ...
        private void BindParentMenu()
        {
            ...
            {//Ankita - 20/may/2016- (For Optimization)
                ...
                string stPmenu = "select PageId,DisplayName from MastPage where Level_Idx=1 and DisplayYN=1 and android='Y' order by DisplayName";
        ...
        {
            string qry = @" SELECT * from(SELECT DisplayName,ViewP,AddP,EditP,DeleteP,ExportP,mrp.PageId,mp.Android  FROM   MastRolePermission_android mrp INNER JOIN mastpage mp ON mrp.pageid = mp.pageid WHERE  mrp.roleid = " + ddlRole.SelectedValue + " AND parent_id = " + ddlModule.SelectedValue + " AND mp.displayyn = 1 and mp.Android='Y' and mrp.app ='"+ddlapp.SelectedValue+"'  " +
                 " UNION ALL " +
           " SELECT DisplayName,  CAST	( 0 AS BIT) as ViewP,  CAST	( 0 AS BIT) as AddP,  CAST	( 0 AS BIT) as EditP,  CAST	( 0 AS BIT) as DeleteP,CAST	( 0 AS BIT) as ExportP,PageId,Android FROM MastPage WHERE PageId NOT IN (SELECT pageid FROM MastRolePermission_android WHERE RoleId=" + ddlRole.SelectedValue + " and app ='" + ddlapp.SelectedValue + "' ) AND Parent_Id=" + ddlModule.SelectedValue + "  AND DisplayYN=1  and Android='Y' ) a ORDER BY a.DisplayName ";
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (GridViewRow gvr in gvData.Rows)
                {
                    ...
                    res = d.InsertUpdatePermissionAndroid
                         (
                ...
                if (cnt < 1)
                {
                    string qry = "update MastRolePermission_android set ViewP=0 where roleid=" + ddlRole.SelectedValue + " and pageid=(select Parent_Id from MastPage where PageId=" + page_ID + ")";
