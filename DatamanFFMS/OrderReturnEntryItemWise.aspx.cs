...
{
    public partial class OrderReturnEntryItemWise : System.Web.UI.Page
    {
        ...
        private void FillDeptControls(int SRetId)
        {
            try
            {
                string str = "";
                ...
                if (SRetId != 0)
                {
                    str = @"select distinct( a.itemid),b.itemname,a.unit,b.stdpack,a.rate as mrp,a.amount as amt,a.SRetDocId,a.cases as qty,a.SRet1Id,a.remarks,c.remarks as remark,a.SRetId,a.BatchNo As BatchNo,a.MFD As MFD,'O' as [type]  from  Temp_TransSalesReturn c left join   Temp_TransSalesReturn1 a on a.visid=c.visid left join mastitem b on  a.itemid=b.itemid where a.SRet1Id=" + SRetId;

                }
                else
                {
                    str = "Select itemid,Max(itemname) as itemname,max(unit) as unit,max(stdpack) as stdpack,max(mrp) as mrp,max(amt) as amt,Max(SRetDocId) as SRetDocId,max(qty) as qty,Max(SRet1Id) as SRet1Id,Max(remarks) as remarks,Max(remark) as remark,Max(SRetId) as SRetId,Max(BatchNo) As BatchNo,Max(MFD)As MFD,Max([type]) as [type]";
                    str = str + " From(select distinct( a.itemid),b.itemname,a.unit,b.stdpack,a.rate as mrp,a.amount as amt,a.SRetDocId,a.cases as qty,a.SRet1Id,a.remarks,c.remarks as remark,a.SRetId,a.BatchNo As BatchNo,a.MFD As MFD,'O' as [type] from  Temp_TransSalesReturn c left join  Temp_TransSalesReturn1 a on a.visid=c.visid left join mastitem b on  a.itemid=b.itemid where a.PartyId=" + Convert.ToInt32(Request.QueryString["PartyId"].ToString()) + " AND c.VISID=" + Convert.ToInt32(Request.QueryString["VisitID"].ToString()) + "";
                    str = str + " Union all ";
                    str = str + " select a.itemid,b.itemname,0.00 as unit,b.stdpack,b.mrp,0.00 as amt,'' as SRetDocId,0.0 as qty,0 as SRet1Id,' 'as remarks,'' as remark,0 as SRetId,'' As BatchNo,'' As MFD,'N' as [type]  from MastItemTemplat a,mastitem b where a.itemid=b.itemid and a.PartyId=" + Convert.ToInt32(Request.QueryString["PartyId"].ToString()) + ") as T Group by T.itemid";


                    //  str = @"select distinct( a.itemid),b.itemname,a.unit,b.stdpack,a.rate as mrp,a.amount as amt,a.OrdDocId,a.cases as qty,a.SRet1Id,a.remarks,c.remark,a.ordid from  temp_transorder c left join   temp_transorder1 a on a.visid=c.visid left join mastitem b on  a.itemid=b.itemid where a.PartyId=" + Convert.ToInt32(Request.QueryString["PartyId"].ToString());
                }
        ...
        private void fillRepeter()
        {
            ...
            string str = @"select m.ItemName,t.* from Temp_TransSalesReturn1 t,MastItem m where t.ItemId=m.ItemId and t.VisId=" + VisitID + " and t.SmId=" + Settings.Instance.DSRSMID + " and t.PartyId=" + PartyId;
        ...
        private void fillRepeteroredr()
        {
            ...
            //string str = @"select * from Temp_TransOrder where  VisId=" + VisitID + " and UserId=" + Settings.Instance.UserID + " and SmId=" + Settings.Instance.DSRSMID + " and PartyId=" + PartyId;
            // string str = @"select * from Temp_TransOrder where  VisId=" + VisitID + " and SmId=" + Settings.Instance.DSRSMID + " and PartyId=" + PartyId;
            string str = "Select itemid,Max(itemname) as itemname,max(unit) as unit,max(stdpack) as stdpack,max(mrp) as mrp,max(amt) as amt,Max(SRetDocId) as SRetDocId,max(qty) as qty,Max(SRet1Id) as SRet1Id,Max(remarks) as remarks,Max(remark) as remark,Max(SRetId) as SRetId,Max(BatchNo) As BatchNo,Max(MFD) As MFD,Max([type]) as [type]";
            str = str + " From(select distinct( a.itemid),b.itemname,a.unit,b.stdpack,a.rate as mrp,a.amount as amt,a.SRetDocId,a.cases as qty,a.SRet1Id,a.remarks,c.remarks as remark,a.SRetId,a.BatchNo As BatchNo,a.MFD As MFD,'O' as [type] from  Temp_TransSalesReturn c left join   Temp_TransSalesReturn1 a on a.visid=c.visid left join mastitem b on  a.itemid=b.itemid where a.PartyId=" + PartyId + " and c.visid=" + VisitID + "";
            str = str + " Union all ";
            str = str + " select a.itemid,b.itemname,0.00 as unit,b.stdpack,b.mrp,0.00 as amt,'' as SRetDocId,0.0 as qty,0 as SRet1Id,' 'as remarks,'' as remark,0 as SRetId,'' As BatchNo,'' As MFD,'N' as [type]  from MastItemTemplat a,mastitem b where a.itemid=b.itemid and a.PartyId=" + PartyId + ") as T Group by T.itemid";
        ...
        private void InsertOrder(int index)
        {
            try
            {
                ...
                HiddenField hiditem1 = (gridorder.Rows[index].FindControl("HiddenField3") as HiddenField);
                ...
                TextBox txtBatch = (gridorder.Rows[index].FindControl("txtBatch") as TextBox);
                TextBox txtMFD = (gridorder.Rows[index].FindControl("txtMFD") as TextBox);
                ...
                if ((amount.Text == "0.00") || ((amount.Text == "0")))
                {
                    //continue;
                }
                ...
                string docID = Settings.GetDocID("ORDSR", DateTime.Now);
                Settings.SetDocID("ORDSR", docID);
                int sno = 0;
                int freeqty = 0;
                int discount = 0;
                int SRet1Id = Convert.ToInt32((DbConnectionDAL.GetScalarValue(CommandType.Text, "select ISNULL(max(SRetId),0)+1 from Temp_TransSalesReturn1")).ToString());
                RetSave = dp.InsertReturnEntryItemWise(Convert.ToInt64(VisitID), docID, Convert.ToInt32(Settings.Instance.UserID), Settings.GetVisitDate(Convert.ToInt32(VisitID)), Convert.ToInt32(Settings.Instance.DSRSMID), PartyId, AreaId, SRet1Id, sno, Convert.ToInt32(hiditem1.Value), Convert.ToDecimal(TotalQty), Convert.ToDecimal(freeqty), Convert.ToDecimal(txtrate.Text.Trim()), Convert.ToDecimal(discount), txtRemarks.Text.Trim(), Convert.ToDecimal(amount.Text), Convert.ToDecimal(Cases1), Convert.ToDecimal(Unit), txtBatch.Text.Trim(), txtMFD.Text.Trim());
        ...
        private void UpdateOrder(int index)
        {
            try
            {
                ...
                int freeqty = 0;
                int discount = 0;
                int ord1id = Convert.ToInt32((DbConnectionDAL.GetScalarValue(CommandType.Text, "select ISNULL(max(SRetId),0)+1 from Temp_TransSalesReturn1")).ToString());

                RetSave = dp.UpdateReturnEntryItemWise(Convert.ToInt64(ViewState["CollId"]), Convert.ToInt64(VisitID), Convert.ToInt32(Settings.Instance.UserID), Settings.GetVisitDate(Convert.ToInt32(VisitID)), Convert.ToInt32(Settings.Instance.DSRSMID), PartyId, AreaId, txtRemarks.Text.Trim(), Convert.ToDecimal(amount.Text), Convert.ToInt32(hidord1id.Value), Convert.ToInt32(hiditem1.Value), Convert.ToDecimal(TotalQty), Convert.ToDecimal(Rate), Convert.ToDecimal(Cases1), Convert.ToDecimal(Unit),txtBatch.Text.Trim(),txtMFD.Text.Trim());

                //}



                //int ord1id, int itemid, decimal qty, decimal rate)
        ...
        {
            string[] SRetId = e.CommandArgument.ToString().Split(';');
            // int id = Convert.ToInt32(e.CommandArgument.ToString());
            int id = Convert.ToInt32(SRetId[0]);
            int id1 = Convert.ToInt32(SRetId[1]);
            int retdel;
            ...
            {
                retdel = dp.delete_itemwiseSaleReturn(Convert.ToString(id), Convert.ToString(id1));
            }
            else
            {
                retdel = dp.delete_itemwiseSaleReturn(Convert.ToString(id), Convert.ToString(ViewState["CollId"]));
            }
            if (retdel >0)
            {
                ...
                if (Convert.ToString(ViewState["CollId"]) != "")
                {
                    dtchkchild = DbConnectionDAL.GetDataTable(CommandType.Text, "select * from Temp_TransSalesReturn1 where SRetId=" + id1 + "");
                }
                else
                {
                    dtchkchild = DbConnectionDAL.GetDataTable(CommandType.Text, "select * from Temp_TransSalesReturn1 where SRetId=" + id1 + "");
                ...
                if (dtchkchild.Rows.Count == 0)
                {
                    string deleteheader = "";
                    //if (Convert.ToString(ViewState["CollId"]) != "")
                    //    deleteheader = "delete from  Temp_TransSalesReturn where SRetId=" +id1 + "";
                    //else
                        deleteheader = "delete from  Temp_TransSalesReturn where SRetId=" + id1 + "";
        ...
        protected void txtMFD_TextChange(object sender, EventArgs e)
        {
            TextBox txtMFD = (TextBox)(gridorder.FindControl("txtMFD")); 
        }
        protected void gridorder_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                  ...
                  TextBox txtMFD = e.Row.FindControl("txtMFD") as TextBox;
                  if (txtMFD.Text == "01/Jan/1900")
                  {
                      txtMFD.Text = DateTime.Parse(DateTime.Now.ToUniversalTime().AddSeconds(19800).ToShortDateString()).ToString("dd/MMM/yyyy");
                  }
                    if (hid.Value == "0")
                    {
                    ...
                    string str = @"select distinct( a.itemid),b.itemname,a.unit,b.stdpack,a.rate as mrp,a.amount as amt,a.SRetDocId,a.cases as qty,a.SRet1Id,a.remarks,c.remarks as remark from  Temp_TransSalesReturn c left join   Temp_TransSalesReturn1 a on a.visid=c.visid left join mastitem b on  a.itemid=b.itemid where a.visid=" + Convert.ToInt32(Request.QueryString["VisitID"].ToString()) + " and a.partyid = " + Convert.ToInt32(Request.QueryString["PartyId"].ToString()) + "";
                    DataTable deptValueDt = DbConnectionDAL.GetDataTable(CommandType.Text, str);
                    if (deptValueDt.Rows.Count > 0)
                    {
