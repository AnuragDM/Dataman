...
{
    public partial class DSRReportL2 : System.Web.UI.Page
    {
         int uid = 0;
         int smID = 0;
                ...
                if (smId != "")
                {
                    ...
                    if (dtbeats.Rows.Count > 0)
                    {
                        ...
                        if (Settings.Instance.OrderEntryType == "1")
                        {
                            ...
                            QryOrder = @"select '' AS COMPTID, convert (varchar,os.VDate,106) as VisitDate,'Order' as Stype,p.PartyId,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson,'' as productClass,'' as Segment, '' as MaterialGroup,0 AS Qty, '' AS Rate,sum(CONVERT(numeric(18,2), os.OrderAmount)) as Value,
                            os.Remarks,'' as CompItem,0 as CompQty,0 as ComRate,b.AreaName as Beat ,cp.SMName as L2Name,cp1.SMName as L3Name, '' as NextVisitDate, '' as NextVisitTime,'' as Image,'' as CompName, 0 as Discount,'' as BrandActivity, '' as MeetActivity,'' as RoadShow,'' as Scheme,'' as OtherGeneralInfo,'' as OtherActivity, 0 AS stock,os.Latitude,os.Longitude,os.Address,os.Mobile_Created_date from TransOrder os LEFT Join Mastparty p on p.PartyId=os.PartyId left join MastArea b on b.AreaId=p.AreaId 
                           ...
                           " group by p.PartyName, os.VDate,os.PartyId,os.Remarks ,p.PartyId,p.Address1,p.Mobile,p.ContactPerson,b.AreaName,cp.SMName,p.Created_Date,cp1.SMName,os.Latitude,os.Longitude,os.Address,os.Mobile_Created_date";
                        }
                        else
                        {
                            QryOrder = @"select '' AS COMPTID, convert (varchar,os.VDate,106) as VisitDate,'Order' as Stype,p.PartyId,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson,'' as productClass,'' as Segment, '' as MaterialGroup,0 AS Qty, '' AS Rate,sum(CONVERT(numeric(18,2), os.Qty*os.Rate)) as Value,
                           os1.Remarks,i.Itemname as CompItem,os.Qty as CompQty,os.Rate as ComRate,b.AreaName as Beat ,cp.SMName as L2Name,cp1.SMName as L3Name, '' as NextVisitDate, '' as NextVisitTime,os1.ImgUrl as Image,'' as CompName, 0 as Discount,'' as BrandActivity, '' as MeetActivity,'' as RoadShow,'' as Scheme,'' as OtherGeneralInfo,'' as OtherActivity, 0 AS stock,os.Latitude,os.Longitude,os.Address,os.Mobile_Created_date from TransOrder1 os LEFT JOIN transorder os1 ON os.orddocid=os1.orddocid LEFT Join Mastparty p on p.PartyId=os.PartyId left join MastArea b on b.AreaId=p.AreaId left join mastitem i on i.ItemId=os.ItemId
                           left join TransVisit vl1 on vl1.SMId=os.SMId and os.VDate =vl1.VDate left join MastSalesRep cp on cp.SMId=vl1.WithUserId left Join MastSalesRep cp1 on cp1.SMId=vl1.nWithUserId where os.SMId in (" + smId + ") and os.VDate='" + Settings.dateformat(VDate) + "' and os.PartyId in (select pp.PartyId from MastParty pp where pp.AreaId in (" + beat + ")) " +
                           " group by p.PartyName,p.Address1,p.Mobile,p.ContactPerson,os1.ImgUrl, os.VDate,os.PartyId,os1.Remarks,i.Itemname,os.Qty,os.Rate,p.PartyId,b.AreaName,cp.SMName,p.Created_Date,cp1.SMName,os.Latitude,os.Longitude,os.Address,os.Mobile_Created_date";
                        }

                        QryDemo = @"select '' AS COMPTID, CONVERT (varchar, d.VDate,106) as VisitDate,'Demo' as Stype,d.PartyId ,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson,ic.name AS productClass,ms.name AS Segment,i.ItemName as [MaterialGroup],0 as Qty,0 as Rate,0 as Value,d.Remarks,
                   ...
                   (select pp.PartyId from MastParty pp where pp.AreaId in (" + beat + ") ) and d.VDate='" + Settings.dateformat(VDate) + "' and d.SMId in (" + smId + ")";
                        ...
                        QryFv = @"select '' AS COMPTID, CONVERT (varchar,fv.VDate,106) as VisitDate,'party FailedVisit' as Stype,p.PartyId,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson,'' as productClass,'' as Segment,'' as MaterialGroup,0 as Qty,0 as Rate,0 as value,fv.Remarks as Remarks,'' as CompItem,0 as CompQty,0 as ComRate,
                   ...
                   (select pp.PartyId from MastParty pp where pp.AreaId in (" + beat + ") and pp.PartyDist=0) and fv.SMId in (" + smId + ") and fv.VDate='" + Settings.dateformat(VDate) + "' ";
                        ...
                        QryComp = @"select TC.COMPTID, CONVERT (varchar, tc.VDate,106) as VisitDate,'Competitor' as Stype,tc.PartyId ,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson, '' AS productClass,'' AS Segment,'' as [MaterialGroup],0 as Qty,0 as Rate,0 as Value,tc.Remarks AS Remarks,
                   ...
                   (select pp.PartyId from MastParty pp where pp.AreaId in (" + beat + ")) and tc.VDate='" + Settings.dateformat(VDate) + "' and tc.SMId in (" + smId + ")";
                        ...
                        QryPartyCollection = @" SELECT '' AS COMPTID, convert (varchar,tc.PaymentDate,106) as VisitDate,'Party Collection' as Stype,p.PartyId, p.partyname AS partyname,p.Address1,p.Mobile,p.ContactPerson,'' as productClass,'' as Segment,'' AS MaterialGroup,0 AS Qty, '' AS Rate,sum(Convert(numeric(18,2), tc.Amount)) AS Value,
                   ...
                   AND vl1.VDate=tc.PaymentDate left join MastSalesRep cp on cp.SMId=vl1.WithUserId left Join MastSalesRep cp1 on cp1.SMId=vl1.nWithUserId where tc.SMId in (" + smId + ") and tc.PaymentDate='" + Settings.dateformat(VDate) + "' and tc.PartyId in (select pp.PartyId from MastParty pp where pp.AreaId in (" + beat + ")) and p.partyDist=0 and vl1.Lock=1 group by tc.PaymentDate,p.PartyId,p.partyName,p.Address1,p.Mobile,p.ContactPerson,b.AreaName,tc.Amount,tc.Latitude,tc.Longitude,tc.Address,tc.Mobile_Created_date ";
                        ...
                        QryDistCollection = @" SELECT '' AS COMPTID, convert (varchar,Dc.PaymentDate,106) as VisitDate,'Distributor Collection' as Stype,p.PartyId,p.partyname AS partyname,p.Address1,p.Mobile,p.ContactPerson,'' as productClass,'' as Segment,'' AS MaterialGroup,0 AS Qty, '' AS Rate,sum(CONVERT(numeric(18,2), dc.Amount)) as Value,
                   max(dc.Remarks) AS Remarks,'' AS CompItem,0 as CompQty,0 as ComRate,b.AreaName as Beat ,max(cp.SMName) as L2Name,max(cp1.SMName) as L3Name,'' as NextVisitDate, '' as NextVisitTime,'' as Image,'' as CompName, 0 as Discount,'' as BrandActivity, '' as MeetActivity,'' as RoadShow,'' as Scheme,'' as OtherGeneralInfo,'' as OtherActivity, 0 AS stock,Dc.Latitude,Dc.Longitude,Dc.Address,Dc.Mobile_Created_date FROM DistributerCollection Dc LEFT JOIN mastparty p ON Dc.DistId=p.PartyId left join MastArea b on b.AreaId=p.cityid LEFT JOIN TransVisit vl1 on vl1.SMId=dc.SMId
                   AND vl1.VDate=dc.PaymentDate left join MastSalesRep cp on cp.SMId=vl1.WithUserId left Join MastSalesRep cp1 on cp1.SMId=vl1.nWithUserId where Dc.SMId in (" + smId + ") and dc.PaymentDate='" + Settings.dateformat(VDate) + "' and dc.DistId in (select pp.PartyId from MastParty pp where pp.CityId in (" + beat + ")) and p.PartyDist=1 and vl1.Lock=1 group by dc.PaymentDate,p.PartyId,p.Partyname,p.Address1,p.Mobile,p.ContactPerson,b.AreaName,dc.Amount,Dc.Latitude,Dc.Longitude,Dc.Address,DC.Mobile_Created_date ";
                        ...
                        QryDistFv = @"select '' AS COMPTID, CONVERT (varchar,fv.VDate,106) as VisitDate,'Distributor FailedVisit' as Stype,p.PartyId,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson,'' as productClass,'' as Segment,'' as MaterialGroup,0 as Qty,0 as Rate,0 as value,fv.Remarks as Remarks,
                   ...
                   and fv.VDate=vl1.VDate left join MastSalesRep cp on cp.SMId=vl1.WithUserId left Join MastSalesRep cp1 ON cp1.SMId=vl1.nWithUserId where fv.PartyId in (select pp.PartyId from MastParty pp where pp.CityId in (" + beat + ")and pp.PartyDist=1) and fv.SMId in (" + smId + ") and fv.VDate='" + Settings.dateformat(VDate) + "' ";
                        ...
                        QryDistDisc = @"select '' AS COMPTID, CONVERT (varchar,tv.VDate,106) as VisitDate,'Distributor Discussion' as Stype,p.PartyId,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson, '' as productClass,'' as Segment,'' as MaterialGroup,0 as Qty,0 as Rate,0 as value,tv.remarkDist as Remarks,
                   ...
                   and tv.VDate=vl1.VDate left join MastSalesRep cp on cp.SMId=vl1.WithUserId left Join MastSalesRep cp1 ON cp1.SMId=vl1.nWithUserId where tv.DistId in (select pp.PartyId from MastParty pp where pp.CityId in (" + beat + ")and pp.PartyDist=1) and tv.SMId in (" + smId + ") and tv.VDate='" + Settings.dateformat(VDate) + "' ";
                        ...
                        QryRetailerDisc = @"select '' AS COMPTID, CONVERT (varchar,tv.VDate,106) as VisitDate,'Retailer Discussion' as Stype,p.PartyId,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson, '' as productClass,'' as Segment,'' as MaterialGroup,0 as Qty,0 as Rate,0 as value,tv.remarkDist as Remarks,
                   '' as CompItem,0 as CompQty,0 as ComRate,b.AreaName as Beat,cp.SMName as L2Name,cp1.SMName as L3Name,CONVERT (varchar,tv.NextVisitDate,106) as NextVisitDate,tv.NextVisitTime AS NextVisitTime,tv.ImgUrl as Image,'' as CompName, 0 as Discount,'' as BrandActivity, '' as MeetActivity,'' as RoadShow,'' as Scheme,'' as OtherGeneralInfo,'' as OtherActivity, tv.stock,tv.Latitude,tv.Longitude,tv.Address,tv.Mobile_Created_date  from TransVisitDist tv inner join MastParty p ON p.PartyId=tv.DistId left join MastArea b on b.AreaId=p.CityId  left join TransVisit vl1 on vl1.SMId=tv.SMId
                   and tv.VDate=vl1.VDate left join MastSalesRep cp on cp.SMId=vl1.WithUserId left Join MastSalesRep cp1 ON cp1.SMId=vl1.nWithUserId where tv.DistId in (select pp.PartyId from MastParty pp where pp.CityId in (" + beat + ")and pp.PartyDist=0) and tv.SMId in (" + smId + ") and tv.VDate='" + Settings.dateformat(VDate) + "' ";
                        ...
                        QryDistStock = @"select '' AS COMPTID, convert (varchar,os.VDate,106) as VisitDate,'Distributor Stock' as Stype,p.PartyId,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson,'' as productClass,'' as Segment, '' as MaterialGroup,0 AS Qty, '' AS Rate,0 as Value,'' AS Remarks,i.Itemname as CompItem,os.Qty as CompQty,0 as ComRate,b.AreaName as Beat ,cp.SMName as L2Name,cp1.SMName as L3Name, '' as NextVisitDate, '' as NextVisitTime,os.ImgUrl as Image,'' as CompName, 0 as Discount,'' as BrandActivity, '' as MeetActivity,
                   ...
                   " and os.VDate='" + Settings.dateformat(VDate) + "' and os.DistId in (select pp.PartyId from MastParty pp where pp.Cityid in (" + beat + ")) group by p.PartyName,p.Address1,p.Mobile,p.ContactPerson, os.VDate,os.DistId,i.Itemname,os.Qty,p.PartyId,b.AreaName,cp.SMName,p.Created_Date,cp1.SMName,os.Latitude,os.Longitude,os.Address,os.Mobile_Created_date,os.ImgUrl ";
                        ...
                        QryOtherActivity = @"select '' AS COMPTID, CONVERT (varchar,tv.VDate,106) as VisitDate,tv.Type as Stype,0 AS PartyId,'' as Party,'' AS Address1,'' AS Mobile,'' AS ContactPerson, 
            ...
            try
            {
                ...
                " UNION ALL select b.AreaName as City_Name,p.CityId as Beat_Id,(b.AreaName) as Description from Temp_TransVisitDist om inner join MastParty p on p.PartyId=om.DistId inner join MastArea b on b.AreaId=p.CityId  where om.SMId in (" + smId + ") and VDate='" + Settings.dateformat(VDate) + "' AND p.PartyDist=0 group by  b.AreaName,b.AreaId,b.AreaName ,p.CityId " +
                    ...
                    if (dtbeats.Rows.Count > 0)
                    {
                        ...
                        {
                            QryOrder = @"select '' AS COMPTID, convert (varchar,os.VDate,106) as VisitDate,'Order' as Stype,p.PartyId,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson,'' as productClass,'' as Segment, '' as MaterialGroup,0 AS Qty, '' AS Rate,sum(CONVERT(numeric(18,2), os.OrderAmount)) as Value,
                   os.Remarks,'' as CompItem,0 as CompQty,0 as ComRate,b.AreaName as Beat ,cp.SMName as L2Name,cp1.SMName as L3Name, '' as NextVisitDate, '' as NextVisitTime,'' as Image,'' as CompName, 0 as Discount,'' as BrandActivity, '' as MeetActivity,'' as RoadShow,'' as Scheme,'' as OtherGeneralInfo,'' as OtherActivity, 0 AS stock,os.Latitude,os.Longitude,os.Address,os.Mobile_Created_date from Temp_TransOrder os LEFT Join Mastparty p on p.PartyId=os.PartyId left join MastArea b on b.AreaId=p.AreaId 
                   left join TransVisit vl1 on vl1.SMId=os.SMId and os.VDate =vl1.VDate left join MastSalesRep cp on cp.SMId=vl1.WithUserId left Join MastSalesRep cp1 on cp1.SMId=vl1.nWithUserId where os.SMId in (" + smId + ") and os.VDate='" + Settings.dateformat(VDate) + "' and os.PartyId in (select pp.PartyId from MastParty pp where pp.AreaId in (" + beat + ")) " +
                        ...
                        {
                            QryOrder = @"select '' AS COMPTID, convert (varchar,os.VDate,106) as VisitDate,'Order' as Stype,p.PartyId,p.PartyName as Party,p.Address1,p.Mobile,p.ContactPerson,'' as productClass,'' as Segment, '' as MaterialGroup,0 AS Qty, '' AS Rate,sum(CONVERT(numeric(18,2), os.Qty*os.Rate)) as Value,
                           os1.Remarks,i.Itemname as CompItem,os.Qty as CompQty,os.Rate as ComRate,b.AreaName as Beat ,cp.SMName as L2Name,cp1.SMName as L3Name, '' as NextVisitDate, '' as NextVisitTime,os1.ImgUrl as Image,'' as CompName, 0 as Discount,'' as BrandActivity, '' as MeetActivity,'' as RoadShow,'' as Scheme,'' as OtherGeneralInfo,'' as OtherActivity, 0 AS stock,os.Latitude,os.Longitude,os.Address,os.Mobile_Created_date from Temp_TransOrder1 os LEFT JOIN Temp_transorder os1 ON os.OrdId=os1.OrdId LEFT Join Mastparty p on p.PartyId=os.PartyId left join MastArea b on b.AreaId=p.AreaId left join mastitem i on i.ItemId=os.ItemId
                           left join TransVisit vl1 on vl1.SMId=os.SMId and os.VDate =vl1.VDate left join MastSalesRep cp on cp.SMId=vl1.WithUserId left Join MastSalesRep cp1 on cp1.SMId=vl1.nWithUserId where os.SMId in (" + smId + ") and os.VDate='" + Settings.dateformat(VDate) + "' and os.PartyId in (select pp.PartyId from MastParty pp where pp.AreaId in (" + beat + ")) " +
                          " group by p.PartyName,p.Address1,p.Mobile,p.ContactPerson, os.VDate,os.PartyId,os1.ImgUrl,os1.Remarks,i.Itemname,os.Qty,os.Rate,p.PartyId,b.AreaName,cp.SMName,p.Created_Date,cp1.SMName,os.Latitude,os.Longitude,os.Address,os.Mobile_Created_date";
                ...
                and tv.VDate=vl1.VDate left join MastSalesRep cp on cp.SMId=vl1.WithUserId left Join MastSalesRep cp1 ON cp1.SMId=vl1.nWithUserId where tv.SMId in (" + smId + ") and tv.VDate='" + Settings.dateformat(VDate) + "' AND tv.Type IS NOT NULL ";
                    ...
                    {
                        QryOtherActivity = @"select '' AS COMPTID, CONVERT (varchar,tv.VDate,106) as VisitDate,tv.Type as Stype,0 AS PartyId,mp.partyname as Party,'' AS Address1,'' AS Mobile,'' AS ContactPerson, 
                        '' as productClass,'' as Segment,'' as MaterialGroup,0 as Qty,0 as Rate,0 as value,tv.remarkDist as Remarks,'' as CompItem,0 as CompQty,0 as ComRate,'' as Beat,cp.SMName as L2Name,cp1.SMName as L3Name,
                        CONVERT (varchar,tv.NextVisitDate,106) as NextVisitDate,tv.NextVisitTime AS NextVisitTime,tv.ImgUrl as Image,'' as CompName, 0 as Discount,'' as BrandActivity, '' as MeetActivity,'' as RoadShow,'' as Scheme,
                         '' as OtherGeneralInfo,'' as OtherActivity, tv.stock,tv.Latitude,tv.Longitude,tv.Address,tv.Mobile_Created_date from Temp_TransVisitDist tv left join TransVisit vl1 on vl1.SMId=tv.SMId
        ...
        {
            if (pageName == "APPROVAL-L2")
            {
                Response.Redirect("~/RptDailyWorkingApprovalL2.aspx");
            }
            else
            {
                Response.Redirect("~/RptDailyWorkingSummaryL2.aspx");
            }
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=DSRReport-L2.xls");
            Response.Charset = "";
